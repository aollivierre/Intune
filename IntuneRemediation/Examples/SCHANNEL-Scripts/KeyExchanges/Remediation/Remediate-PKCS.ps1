<#
.SYNOPSIS
    Enables PKCS key exchange algorithm
.DESCRIPTION
    Creates or modifies registry settings to enable PKCS key exchange algorithm
...
$logPrefix = "[PKCS-Remediation]"

try {
    # Create the key exchange path if it doesn't exist
    if (-not (Test-Path $keyExchangePath)) {
        Write-Output "$logPrefix Creating registry path for PKCS"
        New-Item -Path $keyExchangePath -Force | Out-Null
    }
    
    # Enable the key exchange
    Write-Output "$logPrefix Setting Enabled=0xffffffff for PKCS"
    Set-ItemProperty -Path $keyExchangePath -Name "Enabled" -Value 0xffffffff -Type DWord -Force
    
    Write-Output "$logPrefix PKCS has been successfully enabled."
    exit 0
} catch {
    Write-Error "$logPrefix Failed to enable PKCS: $_"
    exit 1
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU+/L0l1za+6T9DPetSS5n4llJ
# snKggiB/MIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# FCVPi9YzXjbaOpneW9n5ZJurpZruMA0GCSqGSIb3DQEBAQUABIGAscZ6/QvZ6Dr6
# AvMtIhgeRIsNPIgOPZfjPbh4uTw8tqWxJ1YzSpciuU1RKtF6n4KR+b7RDgGo5ETV
# x0w0aveZKqyrlwW8fB8sm1/ZV2+94pey7ubvC5rXSPym7SBTS0hD4BjZC7fljkQk
# 5ZYjdb9KOUaqlaW8xXRwOykrTywzaYihggMgMIIDHAYJKoZIhvcNAQkGMYIDDTCC
...
# MTU1OTAxWjAvBgkqhkiG9w0BCQQxIgQgNzLxhm0R082WcjzaSNuVRNZ0x4ee4+PK
# o7pK1mbosskwDQYJKoZIhvcNAQEBBQAEggIAmr9usnXDnocKGyjOQB/8gur8SRYw
# 2Qk298gB7FRViogYMjQi/n9V0LDWyE93jEdehGFHM5ZrgJ8Ee5pF5sCLGBn6gyUm
# 4GfAC5WRlmoptzgiF0aKv0HxioW1hbBStmdiFDpAisOgyBb2w7e39KvEMX8x0PQe
# 50fgmYhVk/5JPiVIlE7Ke+HBwpb6HkvdB23vVSoy23RIfH2f7b971QzB+Hg+l7zE
# sLEuzgFRAhJSSxw1r2tLR4LP/i+z+evK6YgyMVsf+Ib01XHX47QrenCK3tKsihUA
# ms+zA/K0V1455GoIw8fLkxqUtA7yI6L2t4H2qrqtgXv4i5kJVQsm5FMfzd1Gr7as
# vbLrU/s3u9sIhRMKAXyE57I9Cb/yO1zVq/yKHrcrHFQIaZ8ndQqTxpCtxeWkm7Us
# QXC2Oumexv3vwrhiz9QXcFJqdq8AARk85BDZNej7vjlzi9O34hS7kQ7vCaXxeAp0
# N98n2zMUWMyaa+1GrbccDfwMv/EjS/HAVK72pJuBTIMW1UR6OFVnE1dOkD16NqXR
# 5c1xxNvJ9irng5gdK9cJkwpXavSXeCDBPuOwnlfzy+QvuJvQjoHGfLhgb/K7JlaK
# Bj2JhZp2777vHl7Om13YCwICmzTzRK4gcoFlaxEGNlwNTXSZFFWKNMYizDSFYIZ8
# vwYie4d6q9V5bkc=
