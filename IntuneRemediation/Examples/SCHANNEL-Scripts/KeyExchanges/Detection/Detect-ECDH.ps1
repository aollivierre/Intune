<#
.SYNOPSIS
    Detects if ECDH key exchange algorithm is properly enabled
.DESCRIPTION
    Checks registry to verify ECDH key exchange algorithm is explicitly enabled
    Used for Intune Proactive Remediations
.NOTES
    Author: Security Admin
    Version: 1.0
    Exit Code 0: ECDH is properly configured and enabled
    Exit Code 1: ECDH is improperly configured or disabled
#>

$keyExchangeName = "ECDH"
$keyExchangePath = "HKLM:\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\KeyExchangeAlgorithms\$keyExchangeName"
$logPrefix = "[ECDH-Detection]"

try {
    # Check if the key exchange registry path exists
    if (-not (Test-Path $keyExchangePath)) {
        Write-Output "$logPrefix Registry path not found. ECDH not explicitly configured."
        exit 1
    }
    ...
    if ($enabled -eq 0xffffffff -or $enabled -eq 1) {
        Write-Output "$logPrefix ECDH is properly configured and enabled."
        exit 0
    } else {
        Write-Output "$logPrefix ECDH is improperly configured. Enabled=$enabled"
        exit 1
    }
} catch {
    Write-Error "$logPrefix Error checking ECDH configuration: $_"
    exit 1
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUwbihDr84NcoVMWjFU/rU8JN0
# dHqggiB/MIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# FN/+H/+FrhmndOa+M9WAJohnwmPiMA0GCSqGSIb3DQEBAQUABIGAZPOciFhZTgfX
# 0l+QbEDUiRPu773y/l2dtgDE4SdDZA9Y1t2sZCii9pw+23QWKfsQgAW+msOngy3F
# tktIVL8bpXDgzBHo18oLYuOzY7L5wv5EqXwmVp4xVBtqF7dgy+/hlS5TCzRgy07s
# g66D/VMK7HL0kWa3Fv3fn2eB5YXQ4a+hggMgMIIDHAYJKoZIhvcNAQkGMYIDDTCC
...
# MTU1OTAwWjAvBgkqhkiG9w0BCQQxIgQgzupJx62AbAnpwQYXLRB8K0NU8rTdCVTe
# 83xMUDfSZu8wDQYJKoZIhvcNAQEBBQAEggIAuK8IzMLQNuCEoGKTEexJFR3M5Sim
# l5FadwHIufuY4RFubk5juKNPBaSVwNR5dD7YjtC0zhiLiQ2KUEQmnon4YqJrGEJh
# Es5E2v5Safl3Wj2iXJelmvVA+KI+7hIMOM0rWIT2zptVUrd8OV/OafwG1ymwPtDm
# I6PTlEt0PaYMZ7NbwaZVi99scUw+jnkucVqg52BTCh54Z+zc5Lw2nlHbh7hGKPPx
# 0actP5HpSa87Qfo9DEVSx11NRnb5HmH9daBwh8ytO7/LhapYgo+4PGN4THp4i/QC
# 80sOH9qlb1jnrxgSaNPvyxRePyRPTzcgFFFaApjyfppEOsy3eG1Q5p/5OQPhEVdS
# MUDik145bP+mokSqKOZSPKkZACIRMAWqVRH1C6txDYQgxktTPk9Q1k93w6H4Okas
# uRX7Q0ovwARJXFvryRkXR5H05k0dr6BezhANgoTpNUvQjdsoXsUyb6jHSAtMv2kJ
# nv8533Cv7IAv9VZQFnjt77bJofrjYGUxa/BzLl2cRe+mjZH7LlT7ImbsiYTMe6Fv
# T7PxUkP5I2uEbrQnhskRL0qnWbR3AD/7Rrmo4J7lYot5k/84CTMl3qsR/nkoYzN1
# Sa01Ir8YhnOSWT3sYM9omCgkH14K8lrS1AYa9BAm3tMMBKlCTGi/AYoryXmtpCER
# fPzDX9jXOPDKF30=
