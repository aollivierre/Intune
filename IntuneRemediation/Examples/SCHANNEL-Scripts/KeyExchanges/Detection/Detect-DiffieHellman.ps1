<#
.SYNOPSIS
    Detects if Diffie-Hellman key exchange algorithm is properly enabled
.DESCRIPTION
    Checks registry to verify Diffie-Hellman key exchange algorithm is explicitly enabled
    Used for Intune Proactive Remediations
.NOTES
    Author: Security Admin
    Version: 1.0
    Exit Code 0: Diffie-Hellman is properly configured and enabled
    Exit Code 1: Diffie-Hellman is improperly configured or disabled
#>

$keyExchangeName = "Diffie-Hellman"
$keyExchangePath = "HKLM:\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\KeyExchangeAlgorithms\$keyExchangeName"
$logPrefix = "[DiffieHellman-Detection]"

try {
    # Check if the key exchange registry path exists
    if (-not (Test-Path $keyExchangePath)) {
        Write-Output "$logPrefix Registry path not found. Diffie-Hellman not explicitly configured."
        exit 1
    }

    # Check Enabled property
    $enabled = (Get-ItemProperty -Path $keyExchangePath -ErrorAction Stop).Enabled
    
    # Verify correct configuration (Enabled=0xffffffff or 1)
    if ($enabled -eq 0xffffffff -or $enabled -eq 1) {
        Write-Output "$logPrefix Diffie-Hellman is properly configured and enabled."
        exit 0
    } else {
        Write-Output "$logPrefix Diffie-Hellman is improperly configured. Enabled=$enabled"
        exit 1
    }
} catch {
    Write-Error "$logPrefix Error checking Diffie-Hellman configuration: $_"
    exit 1
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUH3n8x6bKTyx31Gt5EuiwAA/G
# Fr+ggiB/MIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# FLa0WDiRxQLSDZ9JUhjcnhpeglevMA0GCSqGSIb3DQEBAQUABIGAbmgMmxrbm1F4
# nwymM/gded6KZL5vsjq+s3BdbgzIAuaImlzu8LIrMSlfh7YxV5NWlwRwNz/KB5nU
# yhPVQKUOTRO4lo/Pfkdb0+OV8A0T8UYPlP+R0rr1Wh0ZjlIrhDQl3p+oA2Llz3xU
# 0evu6UoL46GHuMuzfQ8HZ40M7XHpacehggMgMIIDHAYJKoZIhvcNAQkGMYIDDTCC
...
# MTU1OTAwWjAvBgkqhkiG9w0BCQQxIgQg1nY3Nm+yTTTFu6YbYl4W09k1KRApoFV5
# bae0B4ZEDAEwDQYJKoZIhvcNAQEBBQAEggIAFMtuJnWe4YFpgYI+0rO7Y/ERDff/
# djXwx/wvEifZMAwYY/PK+ROmFLghWjaxdtsGarslb1opHoLvCkCL4A9jmYfNuMrA
# 0GEjkjIfsIXmOXrkhDAQCSY6i5/kOUA4Mvo/WXS+XdVSR/69lKb+JdtvrevQLML+
# oP0BjoXk+DuAnCdej6nvmSYw5l+d9r2pbON9xXJ6R4zv38H7sky0sofR+dr7LfJE
# Hb/++1/TA4N+AhrPGdpWAYz4BJBNu26x/DxLghYkNt44SE0ZTROTeuymOXuHZi43
# K1uPAe9GmSIU+5fV+dyhwDfdl8V/Mvd7kjC/6fuP+sui8uxqt29p08JUFRJPPIZh
# hDbk7UA0nCGQuuwv4Tn8MOYaOQDkWZ46gO5yRzXLXxuYkRAnK6Q5O9XuR2DafnQI
# FTURvVgQmp4CmEK7KnNE1pCqE2JxxL427oiVl/vV4q7URuVTyXzYP+FZAEpppW23
# m98l3fALzKs8qvxpbiu5PtAfdIe1ntqT+1Odl+gQTjH5KuhhiDvGw35o3qMOiikL
# OlvCqU7CYWCgTL4IJz3lzsopetYeojmIC/ZV8GC20ZOHkm8HjZiJLMjp44iuUv0z
# VNjPQCjK38PvAbQcn4i/jaHXHo8nmq+KBs3lFFiOo/XTrBJfsYizUUzQgxCo1yiH
# ptlhcVdurfv+FT8=
