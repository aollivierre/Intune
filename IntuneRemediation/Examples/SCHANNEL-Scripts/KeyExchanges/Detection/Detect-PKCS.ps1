<#
.SYNOPSIS
    Detects if PKCS key exchange algorithm is properly enabled
.DESCRIPTION
    Checks registry to verify PKCS key exchange algorithm is explicitly enabled
    Used for Intune Proactive Remediations
.NOTES
    Author: Security Admin
    Version: 1.0
    Exit Code 0: PKCS is properly configured and enabled
    Exit Code 1: PKCS is improperly configured or disabled
#>

$keyExchangeName = "PKCS"
$keyExchangePath = "HKLM:\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\KeyExchangeAlgorithms\$keyExchangeName"
$logPrefix = "[PKCS-Detection]"

try {
    # Check if the key exchange registry path exists
    if (-not (Test-Path $keyExchangePath)) {
        Write-Output "$logPrefix Registry path not found. PKCS not explicitly configured."
        exit 1
    }
    ...
    if ($enabled -eq 0xffffffff -or $enabled -eq 1) {
        Write-Output "$logPrefix PKCS is properly configured and enabled."
        exit 0
    } else {
        Write-Output "$logPrefix PKCS is improperly configured. Enabled=$enabled"
        exit 1
    }
} catch {
    Write-Error "$logPrefix Error checking PKCS configuration: $_"
    exit 1
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU/Rip1TNg7B6ipHPga9xjGPps
# 42iggiB/MIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# FNneOuMWE/B9ura5m+CXRjxDcPRuMA0GCSqGSIb3DQEBAQUABIGALGg3A216vAVT
# +fDQ7y9C5iYaxR8uvT6Iy/ehABGgtCQO+o9ZWvXpXzuQryrU4LuWO+ClcEuFVNTH
# XbLpDc8yXVeejJ4xpsK+4ODKSJWz9ligLcRpS0HGGeBA+PJTQeoRMip3BmXeitlY
# R0S3sjzqJj7zBlA1tnf4XWG3zcUcrtahggMgMIIDHAYJKoZIhvcNAQkGMYIDDTCC
...
# MTU1OTAxWjAvBgkqhkiG9w0BCQQxIgQgHCKmTF9IUS7EITeg9BZfq5YfJPpxDvsw
# bUi1yN4AEPgwDQYJKoZIhvcNAQEBBQAEggIAJqDu5bw6Equ28R6l86QKc1RAzZVP
# i3SEx/MPQptCeJDdB0Ifh2vOiqiN0dTaTh3TEPyDj2Gk1RFitGijMpWVm9+W8iTs
# gdZV75uOAAgE5Hbd3NSx+QmEZmwHtaW156lsKkpDlfbL9LSIp99eyrIENfSd1Pfv
# hbQLK1IwONmaMyRwHwYvnehO6rOrQRsu8oAQjHKZX7bAuz5RL3IixWc0ximNF6Qm
# bcorkKAd8SFfWVm0dDwm0NpoYXlljdtNSTgZb1AYrwy5lwMk/hCnTTjBqZxl8DXO
# uRqJUubsZaKKsUIPNCvoyHbgTdziaBDDgpM3BwSAnQE8yOlfH8gqk7innaANV6Eg
# HbZHDNIXIFV9ZPePMXivCb/POk+c9E1Bg55cHCQ/j7wv/Shf3c5jYZZXPiLrDfKu
# LVXzzbqkVDr9lSdWigeaKcvndFnnyIYx6fWdLMagq55SCD4a+B+RIpKCFDeEluqQ
# RWDF09+iFxLxjkCnCbLbaCx/D8yW+aBgbwIuBTM/5sH9YlPg/a5sSkJ1Xu8x8EL5
# Z9ukRoGJ1qd6BYQiyWFQKAIHVIzuu+uH0vCe8it/dRblPxK42Ro224PI86cWKSd9
# qhBcfQJ/7Z8w6aGeRR/Tt6YjNsxZLz1LIU6BvpTLrS6tz1P9GrktTmzisPYIt1bj
# sm3wQCDrThzheCw=
