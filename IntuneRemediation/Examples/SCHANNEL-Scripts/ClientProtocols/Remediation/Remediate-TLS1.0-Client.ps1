<#
.SYNOPSIS
    Disables TLS 1.0 protocol for the client
.DESCRIPTION
    Creates or modifies registry settings to disable TLS 1.0 for client
...
$logPrefix = "[TLS1.0-Client-Remediation]"

try {
    # Create the protocol path if it doesn't exist
    if (-not (Test-Path $protocolPath)) {
        Write-Output "$logPrefix Creating registry path for TLS 1.0 Client"
        New-Item -Path $protocolPath -Force | Out-Null
    }
    
    # Disable the protocol
    Write-Output "$logPrefix Setting Enabled=0 for TLS 1.0 Client"
    Set-ItemProperty -Path $protocolPath -Name "Enabled" -Value 0 -Type DWord -Force
    
    # Ensure it's disabled by default
    Write-Output "$logPrefix Setting DisabledByDefault=1 for TLS 1.0 Client"
    Set-ItemProperty -Path $protocolPath -Name "DisabledByDefault" -Value 1 -Type DWord -Force
    
    Write-Output "$logPrefix TLS 1.0 has been successfully disabled for client."
    exit 0
} catch {
    Write-Error "$logPrefix Failed to disable TLS 1.0 for client: $_"
    exit 1
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUv84JB7ooOMk8VeYM08QXNlQK
# IGmggiB/MIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# FE5p/s7FCDiWf1yWo1c96obVsX9SMA0GCSqGSIb3DQEBAQUABIGAjDXQ1b1wvb1O
# R3QdzJA+9sdGuKkFWikVVCEbDahF9hslrCKF+BCZG5cwFhQFeHUfQRvsq5uSX1fD
# osTeSu7+8p2KIaYOhsBTAvT03Gh2DMZPeekEkiZSw9l7H+jo6JLzDyRzz6Q/4fLN
# iEvjFb7ckscG9EqkDuDCkptWsrxYQKahggMgMIIDHAYJKoZIhvcNAQkGMYIDDTCC
...
# MTU1ODU0WjAvBgkqhkiG9w0BCQQxIgQgLhpyw5VLJ+0HOmSZ8lu2EVCmermJxkg0
# QdDTbpGH7w8wDQYJKoZIhvcNAQEBBQAEggIAi1BSHw7eF6hUh4DTR0Os2QI2elz8
# zbSd0/n/AGbiSWNYbbfO0Phag1UkSP2MyaU+hP0UjrotkGBfSFkSzMHOgbYusnH2
# xi83DMMj+zzQ4WyGpmFtClCXIM5yr1FlOwuG0YHd1Gvc2y0JvwqyVwAmoQl6Na0M
# 14D9BjzwVLXQ0AHMtpnPJQI0XY1AV+1/qmLXd6hD0cN1wkbgj1CKFjlLZ0CAn//H
# uo6aNjzBM8CKG9xDBWNU1WNmk7w3SE/QImwyy+btxp2A/ChkwlaayASOBEQNzWx6
# 9mqaIo58FpK7W45eN9qpKx6isJOMg5Mrc2iSJietOyjMwuFqttNXHOfEcAq2BztZ
# QQDrzH7X43Bu8jFppeKSTdh23sBfrjDovFSZbrkxKB/h0j60R2EiG80p+WbEb/uc
# lTOaFXUIYI24+jkvupexDObPZUSqdm3lPRxVo2WBXleNKoMNAv0oZLv8rMcEB8Ep
# 7QK6pI2G7JEGwIjziJd7Qg6XgtipiebMu+PYzz5xNegx0FdwktFCoDicYG2kJcZv
# wPWPngnskdDyh7Zxd0yTh8k1TvHECdb/u0thYfULRmB9ijfuXaUYZ0a22l1qqE5t
# SN5HKtwH7xyFXaQSn1Mlv6/mzcTunwfW01Nw05idZ9abGVZkMRi28m98WaMWZlmJ
# p3jD1SJt24Dvlu8=
