<#
.SYNOPSIS
    Enables TLS 1.3 protocol for the client
.DESCRIPTION
    Creates or modifies registry settings to enable TLS 1.3 for client
...
$logPrefix = "[TLS1.3-Client-Remediation]"

try {
    # Create the protocol path if it doesn't exist
    if (-not (Test-Path $protocolPath)) {
        Write-Output "$logPrefix Creating registry path for TLS 1.3 Client"
        New-Item -Path $protocolPath -Force | Out-Null
    }
    
    # Enable the protocol
    Write-Output "$logPrefix Setting Enabled=1 for TLS 1.3 Client"
    Set-ItemProperty -Path $protocolPath -Name "Enabled" -Value 1 -Type DWord -Force
    
    # Ensure it's not disabled by default
    Write-Output "$logPrefix Setting DisabledByDefault=0 for TLS 1.3 Client"
    Set-ItemProperty -Path $protocolPath -Name "DisabledByDefault" -Value 0 -Type DWord -Force
    
    Write-Output "$logPrefix TLS 1.3 has been successfully enabled for client."
    exit 0
} catch {
    Write-Error "$logPrefix Failed to enable TLS 1.3 for client: $_"
    exit 1
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUMsawN3Kyr9bVRNaM4LG3nZei
# JR+ggiB/MIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# FJkpb3Eg0yThJbMvcBQDG484RytKMA0GCSqGSIb3DQEBAQUABIGAfVrZj7QJXkyD
# lPPxZEn0DD7a1mSSDbDFCsyZCZQ8nTL2rTIdNW87jC2YWBalUcO06rNaT7DtfdKF
# XAcL/i59iPvP4v341N4ylfuCnMb2UnTDpXHcWU3mmLxCCJ5jI9uwJFIChQDRN90X
# MQfqF2IrE/QXopRRTQwHovsK0LFgxAGhggMgMIIDHAYJKoZIhvcNAQkGMYIDDTCC
...
# MTU1ODU1WjAvBgkqhkiG9w0BCQQxIgQglxVNQgpILSmibS4B4UuTMNL6bqc/t7T1
# R09sQGpCAZUwDQYJKoZIhvcNAQEBBQAEggIAeqvsThmPKFZnCbCu6pP1MQQgvI4d
# Cnmg4cUa91PoUwx2wLbxZWmzUfMpbwyhtTPsmtmHthCjCecOvPmFg/0T+KLfZsiF
# n6EoFo9qMpwyYXbS2PeTOn/Twvjoeg6Zc0sMo1dYVNGsDgWOKFndvR8YTsmAWR8+
# rfW+g+p/pJ6oee1nYacBBwTYKeXUVruzzLw75cGERqzxY+MC3HIkmBBtmYATPWaR
# CveNDWRxaI7PKumcQFVxj26t/0mmCcMEnqUzv58iqHM7MSDI2BDdG+BcXZKKEK7u
# EAeIgXMX7k71BVTDqevo04j4bH2CM49QWdzgd8zNqDIN8Lnpxvt5QvXQpO61wOUA
# q+Go+3aZGw8IycI2OD7c9knOe5Y5K8XFnVpIRJ4CBkyDzsvhztJyAlvG1eZFIDVH
# fJiYbb2hL6dlJD/CYtrRsSVJHbg9YdeTV5yQtWp2Zx6PxTXAoTZGAXNZfWnFtQCV
# bJNTK3Ii1x3cPpuGSi3gM0CsKCRsgTWHbg3647HeQddcl9wCHBvf4PFBgrRuknKh
# tBPfqPt6tQ2EzajzRb9gDW2W//blFBQ8luhduN2tpkdPRgtWtfqFS/+mSwlAgEVv
# yOQpPAttgIzFpIhsjkI13DQJuDTL9jF3YXBougmJfMnNX+ngcrwQWkBKYq/xxI6j
# g2ei0f2QrxqRx/o=
