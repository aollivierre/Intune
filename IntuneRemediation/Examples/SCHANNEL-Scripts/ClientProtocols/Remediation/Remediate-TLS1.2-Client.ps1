<#
.SYNOPSIS
    Enables TLS 1.2 protocol for the client
.DESCRIPTION
    Creates or modifies registry settings to enable TLS 1.2 for client
...
$logPrefix = "[TLS1.2-Client-Remediation]"

try {
    # Create the protocol path if it doesn't exist
    if (-not (Test-Path $protocolPath)) {
        Write-Output "$logPrefix Creating registry path for TLS 1.2 Client"
        New-Item -Path $protocolPath -Force | Out-Null
    }
    
    # Enable the protocol
    Write-Output "$logPrefix Setting Enabled=1 for TLS 1.2 Client"
    Set-ItemProperty -Path $protocolPath -Name "Enabled" -Value 1 -Type DWord -Force
    
    # Ensure it's not disabled by default
    Write-Output "$logPrefix Setting DisabledByDefault=0 for TLS 1.2 Client"
    Set-ItemProperty -Path $protocolPath -Name "DisabledByDefault" -Value 0 -Type DWord -Force
    
    Write-Output "$logPrefix TLS 1.2 has been successfully enabled for client."
    exit 0
} catch {
    Write-Error "$logPrefix Failed to enable TLS 1.2 for client: $_"
    exit 1
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUuPveE47CLagNIW4sbiSHg7+F
# SDmggiB/MIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# FHHNl66Y6i9AoSKdveezMroYyjt8MA0GCSqGSIb3DQEBAQUABIGAi5hAtBjdskT8
# r+DpWbME6HCYuInHf+aX4dYHW1lw41lEgxlq4oPYLJCgNwsAf/3KbY05LiEfno7i
# sHLxePy9azHWblMaccHafj+r9eoIY42kywb30EzThe2TXk9epqdNiFcHyEdsWKqw
# IduzslBXG5XvGarMDWaPnUn8bN44GXShggMgMIIDHAYJKoZIhvcNAQkGMYIDDTCC
...
# MTU1ODU1WjAvBgkqhkiG9w0BCQQxIgQggpsW5cZkkui+pNkmk0ttZHFU8A49B/y2
# oKy7SlivTIIwDQYJKoZIhvcNAQEBBQAEggIAGPvVl0/hoyCB4y0i688UQcKUjFGU
# gb1drLA57PWmRS1go+l9ULaYPu4AW3M88Qb74YsYZ3UxYnPn5FmSMN+DfuhcuP5n
# 4+ym8MBj0H0gs7zPjUNLsIc5chiedAxHs2RGbX5jmFAXtGzrXLsfPPB8lzpz4V4n
# U6QJXv/jCL4JqkL5sdP0/aoeB0bfGdArejj0Adw6fMtx2x4DsszNbrYQi8ovhOZp
# jlBPSeUSBvg1qv/h0NjFQI4SlFHS5LkMrEiKIYBdBAU19xbn7m/ADLY9b0CUljyp
# +dPmdDodLcgy0520I/YWqNDzZ+JZ1dpYcKJ5FOodDSCgklYXz5olqdLCwrfYSyQU
# CQ23lbI8vhfYGlyFQozFLxsktwYU4mKbm05+olB2d9frCPBcc3n3NRBl/3qyUF/X
# fhxkzSuJdqHZWl0UuDYKlB2nHhZwuQxg4Atcvpzcf6zFBkXiBExMGImUE+/cJbyH
# 0sTtz497mlE3io49eLhrAunVOmh6lrNI8IejXogT4+6j9Egw6D1/rCpIDFvm5gcU
# /9BN2fu9f4fybXLrvu76mUcZEhSKs70kNJUV+t/2GuFiHj9fUu8YActTs7gf0Se4
# QfpiVyuwXoLTbqo2qTCUXbqqasHhhFiAje4kGNCvr5lOkPQtGKy5ghvjryS1KUqh
# P0Y+r9RsiAGp54Q=
