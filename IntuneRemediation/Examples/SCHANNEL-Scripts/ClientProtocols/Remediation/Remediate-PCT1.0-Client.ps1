<#
.SYNOPSIS
    Disables PCT 1.0 protocol for the client
.DESCRIPTION
    Creates or modifies registry settings to disable PCT 1.0 for client
...
$logPrefix = "[PCT1.0-Client-Remediation]"

try {
    # Create the protocol path if it doesn't exist
    if (-not (Test-Path $protocolPath)) {
        Write-Output "$logPrefix Creating registry path for PCT 1.0 Client"
        New-Item -Path $protocolPath -Force | Out-Null
    }
    
    # Disable the protocol
    Write-Output "$logPrefix Setting Enabled=0 for PCT 1.0 Client"
    Set-ItemProperty -Path $protocolPath -Name "Enabled" -Value 0 -Type DWord -Force
    
    # Ensure it's disabled by default
    Write-Output "$logPrefix Setting DisabledByDefault=1 for PCT 1.0 Client"
    Set-ItemProperty -Path $protocolPath -Name "DisabledByDefault" -Value 1 -Type DWord -Force
    
    Write-Output "$logPrefix PCT 1.0 has been successfully disabled for client."
    exit 0
} catch {
    Write-Error "$logPrefix Failed to disable PCT 1.0 for client: $_"
    exit 1
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUp9RbfO/z026N+Ox0wU2GTUIa
# nYOggiB/MIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# FBzv3HIpWop/Vglpxccx6RiOOeoKMA0GCSqGSIb3DQEBAQUABIGAMat/fO542tjD
# z/RQRANRR6z8CIW6GftiITBDlVydKliYbBP7ZJW4sfWTaQJQZAExB5H8ZaASuRa5
# W8c2GEFm0+cnERDAM1jhziitOycyItHOP7QW7KF/vryPW4PEqtyFRAA2rcQHhiu7
# fyvr4je1z3MWJU0Thx6BvmVC582dxOyhggMgMIIDHAYJKoZIhvcNAQkGMYIDDTCC
...
# MTU1ODU0WjAvBgkqhkiG9w0BCQQxIgQg2S84rP8eAUwA9AgrtNnfBB0ObcGuN/JG
# D+omHWzxJLUwDQYJKoZIhvcNAQEBBQAEggIAmh9ns7LqOXUUzCioTcQnfao67bX1
# FledU59HyWLym2Rq0hJ/OJMMPsrARqyjykpYV4AQFD/4BAGoInWH7+lDI1cJWM03
# GCCCcq8qOaZUYYfyWXoIi9HEyv683tNzrO6yT5YKMF5VyFRgJWS/tVQiFlVRhdjw
# 4VtAjtO7FaQdrmcMIjM0qtJ/zzhia7+pcb/nk5XP+29lQFAThKzjom72RSofQNiN
# +M7SjtkEISly1SlFs4KMaIDCrd6Qbqp97a2kpxxGYJ5C9tD77ExWu3QSngwMsu7P
# pswYrBKvi3NZqjcj1MBxuVXOdnCCQ4w3mh4W5m802a2sup3lOW4oKY1itJKPE7jf
# uYQoa/WE+o5OzUS4zAw0Uqxn5DIFYdwrJ2JXqNVN6wa1mxeJVPYRUBEVMHXJuYiS
# wl1LOrdramDWiySUtCMh+A+Zo1m53lLAgdF1d2O9WXH1/LN7Bc71nsmGD020pb4F
# kybyx720Y4g1uWZzGw7umKjq4JbM2Ew8oT6RKL4EekIkOYW983crP1VQE4z4QJYj
# jKBQIzWIbrTrWMJto6zpVFdVevcTblO1RjAZkMj51FVafMy3P8cer/VUv/Ex3NeI
# ET2VhOqOrDTV7+G0JNSuFkRjJs50I/cvkGWzWJqst32dVh1a8QAE9tocIgZTzrTK
# rPjLf0RCAjMGkK8=
