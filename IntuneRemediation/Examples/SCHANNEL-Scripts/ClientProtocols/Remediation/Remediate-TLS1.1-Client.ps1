<#
.SYNOPSIS
    Disables TLS 1.1 protocol for the client
.DESCRIPTION
    Creates or modifies registry settings to disable TLS 1.1 for client
...
$logPrefix = "[TLS1.1-Client-Remediation]"

try {
    # Create the protocol path if it doesn't exist
    if (-not (Test-Path $protocolPath)) {
        Write-Output "$logPrefix Creating registry path for TLS 1.1 Client"
        New-Item -Path $protocolPath -Force | Out-Null
    }
    
    # Disable the protocol
    Write-Output "$logPrefix Setting Enabled=0 for TLS 1.1 Client"
    Set-ItemProperty -Path $protocolPath -Name "Enabled" -Value 0 -Type DWord -Force
    
    # Ensure it's disabled by default
    Write-Output "$logPrefix Setting DisabledByDefault=1 for TLS 1.1 Client"
    Set-ItemProperty -Path $protocolPath -Name "DisabledByDefault" -Value 1 -Type DWord -Force
    
    Write-Output "$logPrefix TLS 1.1 has been successfully disabled for client."
    exit 0
} catch {
    Write-Error "$logPrefix Failed to disable TLS 1.1 for client: $_"
    exit 1
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU99oQhG81eBoO5Bx6bfNp1Lpv
# u4mggiB/MIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# FPlcoCWUWVrZy5CoGYy4DM/YLLa3MA0GCSqGSIb3DQEBAQUABIGAA9yjM7qtCLDY
# N8+QToqPeYjGdLUpJqi3OkzjfEx0kNbkQsUGsM/97M4hSKz1XaVqixY5nffLn3O5
# eh58S4DAxCuX4oxcjgAqG27R+wvtksZPkqo+zVe5fbjbqRfU4Poo37O/JBh7Zx/g
# iuzDM19lR5YKvQphSqMXFm8ihOiChYOhggMgMIIDHAYJKoZIhvcNAQkGMYIDDTCC
...
# MTU1ODU1WjAvBgkqhkiG9w0BCQQxIgQgNwVolt1mZGkF3nrGFacl8uYxEUhU1YC1
# N06mQZkg1gYwDQYJKoZIhvcNAQEBBQAEggIAMLfMJ/FYOR0Hb2SqEoTUs/V4lyHM
# oR6gaomPlBvwCkJdIl0hNSouj84J91vf1pdD2RL9NMfbZltZcZVm3ukF51XICMOD
# rIiKwyFXJTjCGYYX9mOCvqKAm8ClADrXtRW677lpGIMsRWmrIWbBO7bq1xCWaTth
# xxgnET8UWMDS8BnFwn2OJx+6GPWDDqgTxUIcSpU4NgV8LtssqaGKCGg9WOKGeqRv
# g+TdEikALhoW6N8/jdkaKfukbYdHR0aICTVkUli6PlAjg2qU4EDpuo/OrLIvNdKT
# baTsDtA00YE80zWOIY/dVuuVn0il1SzoYy4GC48bqcrJCKkEaEuSgb7b7RdjibpM
# Vk13xOZ+5VDDZ8+HOPdXOq1ChSlruDhqBwQC+W5US/C0JSNgGcnXdHPKaqW+FmMR
# WQd/TFI8zppUe92J8v7sFXLIhiEquvzF8mERMXO61gGlk2JEtPDv26XeXxKqDotz
# Oe669Mt5unY/709sTSXv+PukBiroMUaLPLjGPNKpLgT6ATKNxdO+bwotec+x3CDn
# YpNihx7Tp6OFDlc4thbSjGUaQl941si/1YuxinZXJw93IAG6CzPwqpNrbLdqAPGi
# m/ZP33H7Q2qt56ZG3QnwEHdVFEz0I9D8tC0hUn5zg0+7oo14WXvtjusqoff9hwhf
# nL98CKi76uSiiAs=
