<#
.SYNOPSIS
    Disables SSL 3.0 protocol for the client
.DESCRIPTION
    Creates or modifies registry settings to disable SSL 3.0 for client
...
$logPrefix = "[SSL3.0-Client-Remediation]"

try {
    # Create the protocol path if it doesn't exist
    if (-not (Test-Path $protocolPath)) {
        Write-Output "$logPrefix Creating registry path for SSL 3.0 Client"
        New-Item -Path $protocolPath -Force | Out-Null
    }
    
    # Disable the protocol
    Write-Output "$logPrefix Setting Enabled=0 for SSL 3.0 Client"
    Set-ItemProperty -Path $protocolPath -Name "Enabled" -Value 0 -Type DWord -Force
    
    # Ensure it's disabled by default
    Write-Output "$logPrefix Setting DisabledByDefault=1 for SSL 3.0 Client"
    Set-ItemProperty -Path $protocolPath -Name "DisabledByDefault" -Value 1 -Type DWord -Force
    
    Write-Output "$logPrefix SSL 3.0 has been successfully disabled for client."
    exit 0
} catch {
    Write-Error "$logPrefix Failed to disable SSL 3.0 for client: $_"
    exit 1
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU9cDGPGbZqu+3sficdRX7CuBH
# Dh6ggiB/MIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# FAlQZGEV7+O1wiuGWGfg9KhP0SikMA0GCSqGSIb3DQEBAQUABIGABVVlMyqbkhBT
# qIycqiYbl5hqZiLfwV9gtG0orRal4uxvJ6azZ4IVy6WuDvBMYUCLl0bLzBRKkFDB
# eVZiznRarOWWkxkP12T57Sr2YYR8Tgm0rMnwcuwVHQYXFmiuHv09gRBXzaPe+mJg
# Zfrc5JwC2jlQQ5hyPx9NyXlREreZz4ShggMgMIIDHAYJKoZIhvcNAQkGMYIDDTCC
...
# MTU1ODU0WjAvBgkqhkiG9w0BCQQxIgQgTRRk/fygLPDv06dUN4kxRjkbGdHVaf3N
# IbJEN9j96OcwDQYJKoZIhvcNAQEBBQAEggIAOBxxiBArCcIqBwyMz9qjLeHdYTL0
# N6tWt5fqjhC4CWDFiBRLzrmTorQjk5bP/37kxKT+G4jlTnhuPPob6Qzf0YxhFp8i
# YQUBy0rXi92RrQN4QJXHDAfLFVlEGqG9obeXqKyx2BVKX9mt45OPT2yuhf0OF5VQ
# ojViUdcAFkezsSU6cZ0TYLE4BPiSKRaSMaTtmMMRt5g17VKp8u6iBZxRtm2QPSc+
# HNPGjHK+EPYxuqBZMhildhLl5+bsO7k1C5TTLPczA3UPgI/pt6b+YBfVavUICrEl
# ZcJqlSvh34ovd0SdkInZRCtCPEv7poOE9Zi7RqZ0FsvB3Kig7sU2g1zRuKkuDdgy
# JEEkXkUWapnrMmq5+FcWCH8RQlYy//56bnXSKoi60pXwmC4xHPW47vMzDiJcRFMp
# KwQfWfWd0g56g4szjC31BuAmBQXQPecOizUWqaDGZnfjQLSkjiMrmDn7SvmvlKgW
# cQBNdbHQFwWvJSXPvNXJcd/fa4fpbAvcRqnoNqX2UOvhk98ELmTrmPNROBRhkotU
# g825K4MTmJXFa3ZTT+bCyHbWL87RnAQIrfx7fLY7Bu0gaW1rriIJy2sh9jCnSxUu
# REKJoiz8BziQhInKK0yQ5supSgEUGlFkt4J9u5GXiGWSs1UFWfhPkBjwxXzjExNt
# X5K6vhCt0fUUt+I=
