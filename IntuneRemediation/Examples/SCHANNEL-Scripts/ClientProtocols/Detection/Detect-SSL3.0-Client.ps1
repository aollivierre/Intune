<#
.SYNOPSIS
    Detects if SSL 3.0 protocol is properly disabled on the client
.DESCRIPTION
    Checks registry to verify SSL 3.0 is explicitly disabled on the client
    Used for Intune Proactive Remediations
.NOTES
    Author: Security Admin
    Version: 1.0
    Exit Code 0: SSL 3.0 is properly configured and disabled for client
    Exit Code 1: SSL 3.0 is improperly configured or enabled for client
#>
...
$logPrefix = "[SSL3.0-Client-Detection]"

try {
    # Check if the protocol registry path exists
    if (-not (Test-Path $protocolPath)) {
        Write-Output "$logPrefix Registry path not found. SSL 3.0 not explicitly configured."
        exit 1
    }
    ...
    if ($enabled -eq 0 -and $disabledByDefault -eq 1) {
        Write-Output "$logPrefix SSL 3.0 is properly configured and disabled for client."
        exit 0
    } else {
        Write-Output "$logPrefix SSL 3.0 is improperly configured for client. Enabled=$enabled, DisabledByDefault=$disabledByDefault"
        exit 1
    }
} catch {
    Write-Error "$logPrefix Error checking SSL 3.0 configuration: $_"
    exit 1
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUKxrktPeVxzqiIL3Qiy1t+lsI
# JU+ggiB/MIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# FMlHulBd8n51zsAHEa20gP5BiRWlMA0GCSqGSIb3DQEBAQUABIGASXf92RkNyPXj
# /8RGTQhoWytZhGq4PasSY8sGMrqj7FjdENLUNI4CBi2Nn/v7byXCVwdJaWDeK8cA
# qVLw0/2ePuKXn6z8nckSEroVQsSrNifgkGbaOfMsAzRQaLj8ExYIzG508k1Z7eBL
# MLOoSMCUJ7HoXd00AQRE5DicvJvjv5WhggMgMIIDHAYJKoZIhvcNAQkGMYIDDTCC
...
# MTU1ODUzWjAvBgkqhkiG9w0BCQQxIgQgDmx02THtt6s0bQdVFdgFMQBxaIQF9zT+
# FXi2AhchWXswDQYJKoZIhvcNAQEBBQAEggIATNSTeRWKDI6TnlRf7Ku2fR0XANId
# H5NPV1YVaxN6kINPJhivAFDviENX6OCoFL0OwvKZ+/jIuvf78q8aVp5ipRUUatnj
# O/Dq/BRWBuvAcSMYGVa9kTvV2xF7BChux7erjh66wZudEcqsiWRyDMYhTZN311ho
# kAIqVsJ8BKCKkGojmjeVvmz/cG7qfnZJLqp1wiYxzJ4TWvNmB6jWCizcDZ5VaPJI
# dp/GiPv6qmVT1kh6jBEEZWDAOD6KG+bAX02WYWpJ7CbUWqKsOupCtOhqI/GnwQ4/
# M+Hcdey/HZmLDIKQpFIJtE2h5n6B12W5rzcf6V4h9NxXkD5q/8ghkRCREcXn4FWN
# cJCqpmHrY/h3kal+brOplq1FYbETvrVLFfiSiCKI4EGXTgZ/PXkWwrbrhU+b9qWv
# XYdmUqkUUvPOvfQ0dePdx015nOQsT7DLs1NEC0umUc+3P7j4T/u+daIetD8XzHhv
# e8D1rj3aURFbM7oSCcSpbDBvCWG472B+N+JdSRHT8agdstHvHiNWtBfbwEBng2MW
# UGfEQFvGlrXLBwMQgltwi6uPKla58Ub05RHGaJAKGnIrANRLw5iUUQ9jylPh8328
# wo6Eh39QfHrASNhY2d69QIAE80NnKVau/Jhd5jNqnx4WE6up+B7zSXy3IbPK+f97
# WsixN7oNl31VGu4=
