<#
.SYNOPSIS
    Detects if TLS 1.3 protocol is properly enabled on the client
.DESCRIPTION
    Checks registry to verify TLS 1.3 is explicitly enabled on the client
    Used for Intune Proactive Remediations
.NOTES
    Author: Security Admin
    Version: 1.0
    Exit Code 0: TLS 1.3 is properly configured and enabled for client
    Exit Code 1: TLS 1.3 is improperly configured or disabled for client
#>
...
$logPrefix = "[TLS1.3-Client-Detection]"

try {
    # Check if the protocol registry path exists
    if (-not (Test-Path $protocolPath)) {
        Write-Output "$logPrefix Registry path not found. TLS 1.3 not explicitly configured."
        exit 1
    }
    ...
    if ($enabled -eq 1 -and $disabledByDefault -eq 0) {
        Write-Output "$logPrefix TLS 1.3 is properly configured and enabled for client."
        exit 0
    } else {
        Write-Output "$logPrefix TLS 1.3 is improperly configured for client. Enabled=$enabled, DisabledByDefault=$disabledByDefault"
        exit 1
    }
} catch {
    Write-Error "$logPrefix Error checking TLS 1.3 configuration: $_"
    exit 1
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUzfKx0KzLDuqir54yDp4S2EAl
# ejqggiB/MIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# FMLhsG/7JhPUSgdPi0qTs6wYuYm+MA0GCSqGSIb3DQEBAQUABIGAd1bsaUIX1vaZ
# 2osqMnzZVqSMR6LcIzfxGi+zOuDwEaCKKMB739KfEirq7zxVoH9hIC6/rlLu/htn
# nMer3ikwkGh3VWdmboXJkT8ARq1IsUL4PlSjLpO4qc64pD4aC47Hwc0SiciZuR7M
# 5i1V1556TRJQl1Hm/WjleiVtbqO8xrmhggMgMIIDHAYJKoZIhvcNAQkGMYIDDTCC
...
# MTU1ODU0WjAvBgkqhkiG9w0BCQQxIgQgh+C2IbOx8yy3weiVbe8g8pyu7KHl1D/1
# lig0Ic28yVEwDQYJKoZIhvcNAQEBBQAEggIAbV/WMgMjGbANT/2AIUy/rxyzVzSp
# qFWF1ABL2xxDR7y6N9Alr955OYHVoD+8COr1oeJ2Yt8YrMGCFk8Ts8EKOhbD6dEN
# 9cm0ZUu/F/6Of9XFmbTZjCWGxZGxEQQYlclq1ggXIkBUyrYm+HVq8ICPTEd5ek3t
# ck3GkaeCbzOkNuremGfJTQDx5Ceo5txzwFKTk+H2SjZvlDTCaoRpj8AHb1tKg2Yv
# 9x95KG2jXU7+Hj7M6684XugPgomlyTMH2alofBNG17pQRKLidXq0r0EGVEdRBaIj
# E2zOdgbzfm9J/gfvwC6pJiI/nz+xcth0FdpGh2xCrqLP/eqXtm5B7Qn6l8ZcE5nP
# um29fZ51KJ4fA/7Gp7yHkCPoUgvCDcX2/JI9glpMmzn4ixeCLcI35ZHPSDJ97qVI
# pNpdQUnEBFOHZYcsDTC0Ee8s3hCoFmtvzt53GgenZp9gEduXs/1vLlS4INAUDtP0
# Sj8NWWWckhETwiEh2ZlLUbnK/qGdTlJHId0w3UJ8UNHkUvQTdbeUAa3p2XynNy6z
# 4OzrcZ8va3v4buQnfF1Zsu82BbDqTXIQieiBXWIgLEmB1McgN6lG0xqck57xtl8W
# SdMWYrHcgkMSJy0P9+wFkRgjnh5iHRWwfftBs5NBbFCCJmYjAS/jtdCl5yNUjy4Q
# MvSOuobwNo3Iox0=
