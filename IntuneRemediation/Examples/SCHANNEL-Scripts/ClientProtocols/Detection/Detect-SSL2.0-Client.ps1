<#
.SYNOPSIS
    Detects if SSL 2.0 protocol is properly disabled on the client
.DESCRIPTION
    Checks registry to verify SSL 2.0 is explicitly disabled on the client
    Used for Intune Proactive Remediations
.NOTES
    Author: Security Admin
    Version: 1.0
    Exit Code 0: SSL 2.0 is properly configured and disabled for client
    Exit Code 1: SSL 2.0 is improperly configured or enabled for client
#>
...
$logPrefix = "[SSL2.0-Client-Detection]"

try {
    # Check if the protocol registry path exists
    if (-not (Test-Path $protocolPath)) {
        Write-Output "$logPrefix Registry path not found. SSL 2.0 not explicitly configured."
        exit 1
    }
    ...
    if ($enabled -eq 0 -and $disabledByDefault -eq 1) {
        Write-Output "$logPrefix SSL 2.0 is properly configured and disabled for client."
        exit 0
    } else {
        Write-Output "$logPrefix SSL 2.0 is improperly configured for client. Enabled=$enabled, DisabledByDefault=$disabledByDefault"
        exit 1
    }
} catch {
    Write-Error "$logPrefix Error checking SSL 2.0 configuration: $_"
    exit 1
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUr6h9UkPVFLi4ZDkcsmxgZtVT
# L06ggiB/MIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# FMFYo+r1GBGIOu1eiW6reHHufHPrMA0GCSqGSIb3DQEBAQUABIGAG4am2BC/HT0h
# HuK9Fy8s1cpZqTHP+0beRGX5ox/f66XPp0XoPsmZ5bPahFRnvQkMN0TeSlGc25F+
# gCxfByASxeLNf3npIud2LJAvtxmQx5lZIQ39X3X99AY9iWkT+JQNGyjGc7FbkU/3
# MS6reFApFwOVl+xEIq6ikAH/6OhaKkqhggMgMIIDHAYJKoZIhvcNAQkGMYIDDTCC
...
# MTU1ODUzWjAvBgkqhkiG9w0BCQQxIgQg1DTcdRKSWlcpCG1KgGl6CoZSaa+fbH4U
# oMqeR//zdV0wDQYJKoZIhvcNAQEBBQAEggIAlJ4ynSdfOlXCkdaqSPKH+aoRukpB
# CivWOHy9SUGS+CRfvPYolqkA2vTlzL1KfsaI4in1CdnobaUHQX2rn07OAUckCNiB
# LMtwYpuDJEMOX5eTDYIXf9XwhzCPxbxnMV3dEeYSX5PtHhFmRtrZogpbl8t3JfXE
# XODOJfV+g8WfigaIYwHnQDnv2vQ3SHF6l1wO2IYRi+uPkVD/8wCUnGn0n0JxQynm
# FQM82edIzSKzrDKNNggO6cv2NVTlKJph4N5zCoC3mItNdwBWavHgpurS2Ac8sOoS
# bCSYoe1kD8aRRH7hS901QhiZvjQd+d4cuP/Qw37tl2FbZoFXf31BhgZpv5HcxTgv
# /nRmAWuGmtiLGZ+oNAkn9uZQG9XHaca/Q9c+E7FyxaBSsuz40c7rYNjLsgRjR2Hy
# mZSOFjSEMhcKs5x2ViVktiPel/5Jzi4WmzK4VaRm/awl0if/KXEFSfvLbcD7eKfy
# D9nOPa2MX/0Hm6ifC0nAOusfuBukh3Mh2Crd5tScXVAgmtswEEJFs39gu3XP3WMJ
# tk4awHrs9RNdgqAm14aHZEQgCmF38fRIr5N6KGqF43l/IboNHkTnWJMkUvzF3Puq
# VxfhJdJaOneGT3+jliNLliDK3+X0k32vbFXQOOukAhWXSNt9cRvcKCWPAbPK+w0o
# mLKyHurlrGBBdRM=
