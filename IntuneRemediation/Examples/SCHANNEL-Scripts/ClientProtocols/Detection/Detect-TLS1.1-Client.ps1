<#
.SYNOPSIS
    Detects if TLS 1.1 protocol is properly disabled on the client
.DESCRIPTION
    Checks registry to verify TLS 1.1 is explicitly disabled on the client
    Used for Intune Proactive Remediations
.NOTES
    Author: Security Admin
    Version: 1.0
    Exit Code 0: TLS 1.1 is properly configured and disabled for client
    Exit Code 1: TLS 1.1 is improperly configured or enabled for client
#>
...
$logPrefix = "[TLS1.1-Client-Detection]"

try {
    # Check if the protocol registry path exists
    if (-not (Test-Path $protocolPath)) {
        Write-Output "$logPrefix Registry path not found. TLS 1.1 not explicitly configured."
        exit 1
    }
    ...
    if ($enabled -eq 0 -and $disabledByDefault -eq 1) {
        Write-Output "$logPrefix TLS 1.1 is properly configured and disabled for client."
        exit 0
    } else {
        Write-Output "$logPrefix TLS 1.1 is improperly configured for client. Enabled=$enabled, DisabledByDefault=$disabledByDefault"
        exit 1
    }
} catch {
    Write-Error "$logPrefix Error checking TLS 1.1 configuration: $_"
    exit 1
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUtNXOKr88V1YWkPL9EqvJNN1P
# z3aggiB/MIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# FHK09ArBpGtCTxIZBdAL4SeQwlKyMA0GCSqGSIb3DQEBAQUABIGAVq22NkMew4WC
# zdLbPvYulEH/3lKbP6LBs7MXbkDu5xqejgaorVGHAojMIUjGMufoYo9tyYUbw7Rc
# Jrp9Pvt3tMqPsPIGHkMsDPgNCLyEqOzPH1nt0gmQMBzIRS+bn9UvNNwNRS8OeNDn
# JQRBM46Sxwcia2biEXy3IgzO1tCqL+uhggMgMIIDHAYJKoZIhvcNAQkGMYIDDTCC
...
# MTU1ODUzWjAvBgkqhkiG9w0BCQQxIgQgDKfxvORzUBBXY8tXGl4bW6LKYYbH7yOy
# qH8Z5JVJcAwwDQYJKoZIhvcNAQEBBQAEggIApHLl2b6Loav49ar7pc/0ZdxodmIm
# JqongUF2xr0J+FinwiY+yOjGx2bVB9QV/dynrdpORJycrolIqUhe+W51y6rEDBkz
# BQWbg6kn5To9MJp7GY0mbK5m5jIBiI1z6xymKhGTWq7nDI2L7gDar78jKT9tkCWn
# j9NIPToroJwBs8QatFomMjOHYP/3QpkJAlQEO/0xeO0lHLeBvSM+eo93ngK9a3cU
# 41UMEiiyLk5JX4j3Dqs7q9qOp6h78PvbDdxD2lkQmc2l3pyL+9g4qIN1FLFH+Jed
# mIr8Z2NN7aXJol61wIan+NHRLgxYKl5EH0FQ/RhUnRnGRl3e+TuLVdmlxCq2vgKJ
# 4Sl9QJNnQg9mMMWZj/peehHFe910PfYdM6K68b2kS1NtcByOfNpgTBktosOr5g0Z
# B5cqHyVXxQQ1w9X39oTR7wX8nmvDgtrOBFPzJFt4eOMbEcMiaAcXKy1o1VsDzaNW
# yezfLUG3RXMISYDEdRrbEExAJ1NJbE+skGRxmZuTnRoJU9i360NDsk955k5Z102K
# HlmLrY+jFR2BqCtJl/+N6Tf4TA+vZKVXY4U9wnGePrUDW9VqAgulGhF3SzJ+gkaw
# FswiGUMC3ogy3VadBPVFRe4du/LF/RLiHa8WsEi3ENAtohMDozjG+qLQdA22xkCa
# 8824tvzcGPZAHe8=
