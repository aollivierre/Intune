<#
.SYNOPSIS
    Detects if TLS 1.0 protocol is properly disabled on the client
.DESCRIPTION
    Checks registry to verify TLS 1.0 is explicitly disabled on the client
    Used for Intune Proactive Remediations
.NOTES
    Author: Security Admin
    Version: 1.0
    Exit Code 0: TLS 1.0 is properly configured and disabled for client
    Exit Code 1: TLS 1.0 is improperly configured or enabled for client
#>
...
$logPrefix = "[TLS1.0-Client-Detection]"

try {
    # Check if the protocol registry path exists
    if (-not (Test-Path $protocolPath)) {
        Write-Output "$logPrefix Registry path not found. TLS 1.0 not explicitly configured."
        exit 1
    }
    ...
    if ($enabled -eq 0 -and $disabledByDefault -eq 1) {
        Write-Output "$logPrefix TLS 1.0 is properly configured and disabled for client."
        exit 0
    } else {
        Write-Output "$logPrefix TLS 1.0 is improperly configured for client. Enabled=$enabled, DisabledByDefault=$disabledByDefault"
        exit 1
    }
} catch {
    Write-Error "$logPrefix Error checking TLS 1.0 configuration: $_"
    exit 1
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUorG7aP184p2j3kHidPiegIiV
# jsyggiB/MIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# FPIQk3uYh9JrQEs7MuaWm/k62IlQMA0GCSqGSIb3DQEBAQUABIGAeGTlUP5/Mqor
# zZv8tnphgRgHTZsAdnzzWh49Wg/3N4ydXQOSbPvLzZSHTiw08fubJqyQCN8USNiW
# /N9eEihBgHwgg2PHJEAJKDiRhZCUNc2r1YC4hD0UXV0IrUCGqzBl1MjBg1YToh2O
# gB1S2ys3j9mJ/PPV9ctmZpzFTX/5pPyhggMgMIIDHAYJKoZIhvcNAQkGMYIDDTCC
...
# MTU1ODUzWjAvBgkqhkiG9w0BCQQxIgQgM6uPPXgvTjmW2KuQ13LHLAprurcv6Oi+
# NkY3OWBbEOMwDQYJKoZIhvcNAQEBBQAEggIAHarAa3EiP8ppyvN36noUFRDMNvt7
# T7utSFHBOhiAAReHse6WzckbBoZGd/ElmueqmAm9P6+xXY/2a1tNpRmxCkyvJCgh
# IqqDbU8GDMIIEuxyST9VGtSH/IBG1GOOE3MyDlPgpcMGLpp+x5IXVt2eN4NX1Imz
# jsuOVaWYzd3UrSNhmtTUOU1T5kGaG1H1/BGWoLXpZGIeUdheaNBGFX9Yj7rrFMPz
# H3XIokpUvx8eOiy1WZkbHCdR6ZpCu4vJUKVIQlXAbwWTZPYdyiZnOXJpW/j0pvps
# qJe7KfrKNzzVwXY2MFSatixIiJ/I3xftWfXyOVQNfQfB+Ibl+syP9AhYqIUjNYAB
# ScAqwMzg7KgDo80fqvHoVspsugM8NLgmAlyJcSDmYQ6KrWqhGFTGpnh8PNoF4H3v
# h+LQTyCZsajIdBJW4s/bW7eBQdxH94pIpAxbV3l9zqbkZC0W8xj0Fnh9zqpAZ4mh
# Xq9g0ZZ1LGFet7Dxn/gEcb9Tl5kZ+A+N3dgGiYNasx5An6floRmAy0XbAvinuZ5q
# 1Ql5fAgUyCHfgUu5qP3d+W7lP3lSZIdXKK+9ZhyfM3eV/t//lBrMHBxlDBgCkYxV
# uylLd2GY45E2mhpnms5xgYZPMZ2ZoLlHAZDsNj1ULWO3D4fHMLbc+dCJRK77nDMn
# 21mWpCj3KPF8Xsw=
