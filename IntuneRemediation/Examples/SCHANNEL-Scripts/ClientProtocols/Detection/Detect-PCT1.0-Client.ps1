<#
.SYNOPSIS
    Detects if PCT 1.0 protocol is properly disabled on the client
.DESCRIPTION
    Checks registry to verify PCT 1.0 is explicitly disabled on the client
    Used for Intune Proactive Remediations
.NOTES
    Author: Security Admin
    Version: 1.0
    Exit Code 0: PCT 1.0 is properly configured and disabled for client
    Exit Code 1: PCT 1.0 is improperly configured or enabled for client
#>

$protocolName = "PCT 1.0"
$protocolPath = "HKLM:\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Protocols\$protocolName\Client"
$logPrefix = "[PCT1.0-Client-Detection]"

try {
    # Check if the protocol registry path exists
    if (-not (Test-Path $protocolPath)) {
        Write-Output "$logPrefix Registry path not found. PCT 1.0 not explicitly configured."
        exit 1
    }
    ...
    if ($enabled -eq 0 -and $disabledByDefault -eq 1) {
        Write-Output "$logPrefix PCT 1.0 is properly configured and disabled for client."
        exit 0
    } else {
        Write-Output "$logPrefix PCT 1.0 is improperly configured for client. Enabled=$enabled, DisabledByDefault=$disabledByDefault"
        exit 1
    }
} catch {
    Write-Error "$logPrefix Error checking PCT 1.0 configuration: $_"
    exit 1
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUcpwIpyMc0FnQpq1VGmB8RTSI
# 4zeggiB/MIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# FPcvdJiNWNvVEEf1rkd7Yw5sWS4pMA0GCSqGSIb3DQEBAQUABIGAn3wmQRzJelw3
# oeFtIwpaTxK5wewSHftUTTlPNXlMEsRLyZwlGOuWGW7fNH8H1GSg0Pptrha6jUiJ
# QY8oTbNydaEMxSHg/ODA6uooLEamRpSLhSXYpJDNAF/Eq7rOFx+4bDL8B5Dt86FU
# Qie/gxPhmIvs61/b1wOSnN4u0zg+adyhggMgMIIDHAYJKoZIhvcNAQkGMYIDDTCC
...
# MTU1ODUzWjAvBgkqhkiG9w0BCQQxIgQgKclK9T+pxcbRmqAa+A6Hb/gqk5BZSXDU
# gFUNs+0tjX8wDQYJKoZIhvcNAQEBBQAEggIARiXA8BNTeiE4I4Ugow3Jho0oJEuM
# P1eSsvaP3iH6z6vp2AJou/LdPWQbP3cyXT8jqZPTFLz8Pam4MwiBe8DQ/Do1Fgwe
# BIFoxb4JhXRLNdj8V+Ks2wEh+zX5GW3R4D4F5U4WFS7KjpA4EOGg26wujsMG2mqQ
# 5+rl7kD5Q8+T9C9iFkKJhlos7ZRJGKlYwtTy82C6OKK3qzgs7N1/p1h9uSXmLJbY
# Zr2C4HRlGQWbxwXs7Sjgr+yIqdF1QanW3fsuEiFsFRhJxMqqmw37BDe5CnSqwhAz
# 9A+zxtNo0O+WeW/RY7Ysqee5ouTO7UySa4IOC/Drh+k7vGD+mktApz2hXx1NnDmv
# C5tgzM/ZS/6fyTXiPLCu+ZQKL4zcefxQhqZ3/7F8tHUp3+FqS1Wh8kMaizYK0tST
# 25MzktJgzByTK4xxsuIbDoolgV3EzIRa8tw0r9f7EH0W27LNjsbLtLjAZjoUlj1z
# 7WoVzSVDRT54dQEc8nIyO/SXeMob2s740lje5MJdLf0ITT7sTBoOiCGB+XQfNcNA
# 8axRRiBU/RSVrkcsCHT4/P4/KElFR96BEbRCC1ZJ3f+C3GCfOt26b02e6+0mqodv
# Gh1cVxrwAz5PuKaXWqMKCw5bsYKKfhBoAyN9X6ZFIr/Ruyuf5ks8RCHnz0NPPjdN
# TgGb+EeZoir7xxM=
