<#
.SYNOPSIS
    Detects if Multi-Protocol Unified Hello protocol is properly disabled on the client
.DESCRIPTION
    Checks registry to verify Multi-Protocol Unified Hello is explicitly disabled on the client
    Used for Intune Proactive Remediations
.NOTES
    Author: Security Admin
    Version: 1.0
    Exit Code 0: Multi-Protocol Unified Hello is properly configured and disabled for client
    Exit Code 1: Multi-Protocol Unified Hello is improperly configured or enabled for client
#>

$protocolName = "Multi-Protocol Unified Hello"
$protocolPath = "HKLM:\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Protocols\$protocolName\Client"
$logPrefix = "[Multi-ProtocolUnifiedHello-Client-Detection]"

try {
    # Check if the protocol registry path exists
    if (-not (Test-Path $protocolPath)) {
        Write-Output "$logPrefix Registry path not found. Multi-Protocol Unified Hello not explicitly configured."
        exit 1
    }

    # Check Enabled property
    $enabled = (Get-ItemProperty -Path $protocolPath -ErrorAction Stop).Enabled
    
    # Check DisabledByDefault property
    $disabledByDefault = (Get-ItemProperty -Path $protocolPath -ErrorAction Stop).DisabledByDefault
    
    # Verify correct configuration for disabling (Enabled=0, DisabledByDefault=1)
    if ($enabled -eq 0 -and $disabledByDefault -eq 1) {
        Write-Output "$logPrefix Multi-Protocol Unified Hello is properly configured and disabled for client."
        exit 0
    } else {
        Write-Output "$logPrefix Multi-Protocol Unified Hello is improperly configured for client. Enabled=$enabled, DisabledByDefault=$disabledByDefault"
        exit 1
    }
} catch {
    Write-Error "$logPrefix Error checking Multi-Protocol Unified Hello configuration: $_"
    exit 1
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUQfVfjWgB9jPCyey0gS4jcCVe
# 6oCggiB/MIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# FDHDbnmK9fuu1PMMb3njS0xcEr55MA0GCSqGSIb3DQEBAQUABIGAmdo78sda5/eO
# nhAxTROVWyDMkSbEDtb1YzEGv3871yxeDg/RwJmmm8IaEPEdErQ0iKLCs9P8qvx3
# JP7JvmLvcUckXoK9zu50TwqiSdVzvSL5l2BdR0DQMETzjg8Xm6GcnGGtVktzBzMj
# V/flqUgJlNIa9GPtS4z8NUI5pDWPBnehggMgMIIDHAYJKoZIhvcNAQkGMYIDDTCC
...
# GAYJKoZIhvcNAQkDMQsGCSqGSIb3DQEHATAcBgkqhkiG9w0BCQUxDxcNMjUwMzEx
# MTU1ODUzWjAvBgkqhkiG9w0BCQQxIgQgty6v12CPE4Mdr544lklmxbkk0U7kDdFg
# oJAejPtrAE8wDQYJKoZIhvcNAQEBBQAEggIAm2prp0RdLo8iMmAWS2e+kfTUmEHQ
# Z8an3OdxCYKSaUyLwB3Dl4GuxYlxOEP5DFbTequ5uVIrgfoZ8RNp7vBbIu8oteVv
# 8V3GUUkssWQCCUXBZGHl0FFWa9/1mKm1bjlC4e319QcWtJLMbLwjVM5+epgJi9hj
# iAOdIcAF3bZX3DoNvbYtUgBDQuhRMOMkWlMd3wxIRHxwRsYv3kXmIi24+GWvFuB7
# 9EEHoxVkV2aCGVAcbbOVpfCbJdUcN+ZlRI5i7xhrt8h7Af48CKvJIXMjaHHWAYLG
# K0iv80T7CltX6Vb//BtUq/0J4FGCPHFyjTBhTMcFx1C0H9klgyJxom2SB5/TjspQ
# PDGkapMN+pHHgnsFsT6TgraDRzg4k+qR1QLxOAqPESIsXvIoLhCidCxkP58wTGfd
# R0j1l3qYMuD2hq6oN3MiewbFczRN+klWawSwTyuzLdn9CNYW439ANQkwqKiXgjHt
# LvCfvJTrROUqrot9wtQcQ77UFFeNznrbzF5RKaU15DGxNkfzZJ9W+J4am5EIpRiI
# 63j7NW5MxZFCEYBtHe1ab1ErfINY+ZR+yx411/U88BqTAbCkyiHYuB7lTNPtQ8Co
# a9VC1Doq9SSJZrR8prHV04ffygtwRXuyOBNjpY5VJIBT2SA0S59zAa1WaEVQDfm7
# F+/mlSFhWlwIwk4=
