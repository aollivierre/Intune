<#
.SYNOPSIS
    Detects if TLS 1.2 protocol is properly enabled on the client
.DESCRIPTION
    Checks registry to verify TLS 1.2 is explicitly enabled on the client
    Used for Intune Proactive Remediations
.NOTES
    Author: Security Admin
    Version: 1.0
    Exit Code 0: TLS 1.2 is properly configured and enabled for client
    Exit Code 1: TLS 1.2 is improperly configured or disabled for client
#>
...
$logPrefix = "[TLS1.2-Client-Detection]"

try {
    # Check if the protocol registry path exists
    if (-not (Test-Path $protocolPath)) {
        Write-Output "$logPrefix Registry path not found. TLS 1.2 not explicitly configured."
        exit 1
    }
    ...
    # Verify correct configuration (Enabled=1, DisabledByDefault=0)
    if ($enabled -eq 1 -and $disabledByDefault -eq 0) {
        Write-Output "$logPrefix TLS 1.2 is properly configured and enabled for client."
        exit 0
    } else {
        Write-Output "$logPrefix TLS 1.2 is improperly configured for client. Enabled=$enabled, DisabledByDefault=$disabledByDefault"
        exit 1
    }
} catch {
    Write-Error "$logPrefix Error checking TLS 1.2 configuration: $_"
    exit 1
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUhOva0Xk/w5z0fPMfdjwyRwUT
# CI2ggiB/MIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# FDFMBmRJmzZcvHWR2G5XpaCv5E6hMA0GCSqGSIb3DQEBAQUABIGAVvJLPNUCu0Fx
# X2/cyhHC3fUQ0Gk6A6WJm0eUgDdFL6T8ZEwFCItrGbyvZ4riGE7j1YKhBmSAiQ32
# KZd38skUDt1lk6KuOfgTEGVDUcKOvVvB20EANVDFM7wPte8Rmqr+FqPPyLTa4Ttt
# pN1kfr64+iah7nXNCc5UDtpL598FhPehggMgMIIDHAYJKoZIhvcNAQkGMYIDDTCC
...
# MTU1ODU0WjAvBgkqhkiG9w0BCQQxIgQgjPTNJCwBqhcLVT6WJXwkya73q52Ew6pp
# LFyaBQ89GaMwDQYJKoZIhvcNAQEBBQAEggIAcolwkCSZteGOdceOHD7TsyV0rN8Q
# jwFN4LmjqD+8BzkB3pU9Me0/MGD4IUMjIDn9Q9xrSq3ngweNB00Wa+xAJomwwDDM
# b49QgBaF+4D58OmnXbuLFVhaoSTYVxUtdsJXuPw3jNFolEA+fbCBcjBRM4+Ecoy9
# ZbC9ILXwS8NhnPYB4ZT+xNdC2wEGIqBRIXGKy9aGQNEX3F4S7qb9i1UYMmO93Ani
# W4frmi4jwZUdZ/qKXzbHfjT2tshcTfcu0dlV+YVPv3Tu4oSZXC5Jg1VfNIEhpxBu
# HLbYp5lJo66sLjkJ7kHtP9+oe9kycf/sCtVQm9P9QtXLINrOtbXLVCd+5kf8aUWr
# 4jzshIaz0mH6RdAVvoKfgOMrWNdGABRmSDnVqtDfcZH9rwOUTMLtLYEst7y7jjjW
# xMwdAypk5r9nK/YBd52I1USTpHHyLsUKl30uuavcXXau9MNbZiTO68H0xq01a4ay
# z2nLJGrJT1jlsu/WC532W3qWXOB4GBCBbJH8n8xkRuhd6RnBTPYlMruwSGI1i6Qg
# 4g2+vm2TlVdy+RFK65jdEK07srQIVhwlDMZTDWHr2Bpd8Uv00ewQgGkffjBaTvhm
# Uw9lXXFFn+Ko5MTqZiTScKfUaeMsG9fAubisFXU7FrpYzIJ7Q6sQ2Z33VHU8fH36
# IYy/YarLDSaWdmc=
