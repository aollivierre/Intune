<#
.SYNOPSIS
    Detects if SHA512 hash algorithm is properly enabled
.DESCRIPTION
    Checks registry to verify SHA512 hash algorithm is explicitly enabled
    Used for Intune Proactive Remediations
.NOTES
    Author: Security Admin
    Version: 1.0
    Exit Code 0: SHA512 is properly configured and enabled
    Exit Code 1: SHA512 is improperly configured or disabled
#>

$hashName = "SHA512"
$hashPath = "HKLM:\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Hashes\$hashName"
$logPrefix = "[SHA512-Detection]"

try {
    # Check if the hash registry path exists
    if (-not (Test-Path $hashPath)) {
        Write-Output "$logPrefix Registry path not found. SHA512 not explicitly configured."
        exit 1
    }
    ...
    if ($enabled -eq 0xffffffff -or $enabled -eq 1) {
        Write-Output "$logPrefix SHA512 is properly configured and enabled."
        exit 0
    } else {
        Write-Output "$logPrefix SHA512 is improperly configured. Enabled=$enabled"
        exit 1
    }
} catch {
    Write-Error "$logPrefix Error checking SHA512 configuration: $_"
    exit 1
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU+ycg+hlrhI1odroo7cAdFbPW
# +kqggiB/MIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# FEA1KzihTiX+7OyPiipt3xR2baFLMA0GCSqGSIb3DQEBAQUABIGAg2DAv4PgEw6l
# qMo/IPbNVjaURIsa3RYoKduIgAH7wgeL1SIPJ9UVJdw9qttsTlRHXKExqEQ5sfOx
# Yo5i5i2xzicw9vhPGaYxptaN6PBwfkbY350bg020MtQbR57O84D//tey+pJQz/3J
# YypfB0YnY8yoKLKhYqT4FlLQUdYs06qhggMgMIIDHAYJKoZIhvcNAQkGMYIDDTCC
...
# MTU1ODU5WjAvBgkqhkiG9w0BCQQxIgQgb/+tBf5O1sH3f5cNyH3ycUxn4gFN4Kqy
# bjKwiZY5AsQwDQYJKoZIhvcNAQEBBQAEggIAJCpT3i6ude2qbl75cnnzAoOWVd+t
# cD76NfgLTtUNX3IDKloIIcEYpPQkBnTIwDse03bIYW2QnQ3osicrPbMz17tp2htO
# ryXIDoHyytWlbPu0AVAXqyOg5r5giVVLwrE++C2kYG9pMvClnVkLu6PhYsrBaxIE
# i9blqmEB991FYA/yiJmTobyOo8Uz29P+Zedwa3rbHwV2o55BDRJ2FjlJX5EY9Sho
# lE8BP8UQuZRJOdWO+NIsm7a0r/n7nGFJ+hDSlSoyrPjfd9Q4d89QBz+/q1HsYHfk
# kM+2pvQFt2B+UzrafTh2if9QYwYTNUzm5Ae+amSMsPK1z5GnTq/fsJo4E78EqArU
# tjG9yslAgVkHfQvuaBggV7VXqeQHEhPXerZdzZB6YLmbrQ5M9SAJcLXzFzo4qykA
# 4N0ivb1DUrSq0UrhvCPtcjHH1yOP7k4MzcwiSiKdbeEjouNF00Pn3VJzqgvb4NdK
# kP0/Fk1Tnr9wEsmUks+s/hmrNGXvGPL48r/DDNP2NMaxPB9S1/q36w+3r7wXyckY
# er5Sja6tX6CPpaF8TijwSj8AmSqRHgiMYU3Rl9cMhl+y3FqreFAuUghxBrD8xUTj
# 7ZPoA/x9iLO22MARsH6Z8lv7/8Lwziuv9vymGOphvxsBYq5TgAjM6e2lKdK9eUdg
# ms+JI/WMZxaDe+A=
