<#
.SYNOPSIS
    Detects if SHA hash algorithm is properly enabled
.DESCRIPTION
    Checks registry to verify SHA hash algorithm is explicitly enabled
    Used for Intune Proactive Remediations
.NOTES
    Author: Security Admin
    Version: 1.0
    Exit Code 0: SHA is properly configured and enabled
    Exit Code 1: SHA is improperly configured or disabled
#>

$hashName = "SHA"
$hashPath = "HKLM:\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Hashes\$hashName"
$logPrefix = "[SHA-Detection]"

try {
    # Check if the hash registry path exists
    if (-not (Test-Path $hashPath)) {
        Write-Output "$logPrefix Registry path not found. SHA not explicitly configured."
        exit 1
    }
    ...
    if ($enabled -eq 0xffffffff -or $enabled -eq 1) {
        Write-Output "$logPrefix SHA is properly configured and enabled."
        exit 0
    } else {
        Write-Output "$logPrefix SHA is improperly configured. Enabled=$enabled"
        exit 1
    }
} catch {
    Write-Error "$logPrefix Error checking SHA configuration: $_"
    exit 1
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUIDQuJZ+TzM07C3uY4XJeXraS
# P82ggiB/MIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# FEJmFQ5tUlvDtgq9CcHkghMH9AUbMA0GCSqGSIb3DQEBAQUABIGAZqT82l3US0hw
# XqecsCS2N2E0/E3T32OzoKJOuT2ZKuUwv/IzDL4+tifvxjiu4ynCasgNwoEKC20F
# BgOe/oSOvWZ0cP4UpBewTjbP/HMPCxFQBS1hj0QTUXh6M5ofpxqpQuOqvdsy2B0I
# IThPngxEyg7CEmYYAXAJNurhvtSAssehggMgMIIDHAYJKoZIhvcNAQkGMYIDDTCC
...
# MTU1ODU5WjAvBgkqhkiG9w0BCQQxIgQg9A6iuAj2IBOSyH52uVwQsSKENuiDI93s
# v9mqxvluqu8wDQYJKoZIhvcNAQEBBQAEggIAncbMkG1sgaNet5A5EZtcfDsq7rE1
# GgZ5w+gV94dS6rPB+Ryw+jHHy+ZqK2V1ryAYe1txOeRgptSXGhq2TFbyVAtHBaF6
# vx2hpbRLJ9/v5IdKjUmwxBqum99mj8PthmfwCUqxAoG0tcB/EWQPcXnMczZy1IBL
# eeU7XfRkLCA27cwgnScJ+uflJmbX/vD1G75LVUOSRR7fvtfxO5UM5S3bXeyPMCmw
# MhXS4nvvO0mhyIE4UjjGVUapMAamcevVzgkw+f+dfuMIR9WLPZO05+DzOVApgATt
# oMHoMRlS6G2zc40/IahqE5LoirYBixQL1SslfLH4tcyCHHmKim0+sXygtP0FfOHp
# H/JJVpxnofvnYnWoOL8U6ivRQ0v4QH69uwXBTM41+IrAZjGLrI+oxSJfvl3QiP1R
# ekQ+Rs1NBWa7jxaarsVBjeeRoxOJY7iZ9lG74RVTGVaIWf0/t31g7pLNI9Efpphe
# iuEjtg8FpNn8k0Koz9mgeLrFAoBEI2CgffvuEj2VFMlgjWmaLXHy8MvERQNZ9cat
# uK8LopPHk+jpG129L7XMeaIFj2vfaI6APR9PNYoOWdWCYhPxplfcb5CAuBIRcvjH
# VidMsfhvSmvu9ck7fhl+bVyvtYBZrZdaME/9AYlsjM/Dr4KPLPf6S4JHoq0pfFJS
# J3wTQDEpYYhYUQs=
