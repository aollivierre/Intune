<#
.SYNOPSIS
    Detects if SHA384 hash algorithm is properly enabled
.DESCRIPTION
    Checks registry to verify SHA384 hash algorithm is explicitly enabled
    Used for Intune Proactive Remediations
.NOTES
    Author: Security Admin
    Version: 1.0
    Exit Code 0: SHA384 is properly configured and enabled
    Exit Code 1: SHA384 is improperly configured or disabled
#>

$hashName = "SHA384"
$hashPath = "HKLM:\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Hashes\$hashName"
$logPrefix = "[SHA384-Detection]"

try {
    # Check if the hash registry path exists
    if (-not (Test-Path $hashPath)) {
        Write-Output "$logPrefix Registry path not found. SHA384 not explicitly configured."
        exit 1
    }
    ...
    if ($enabled -eq 0xffffffff -or $enabled -eq 1) {
        Write-Output "$logPrefix SHA384 is properly configured and enabled."
        exit 0
    } else {
        Write-Output "$logPrefix SHA384 is improperly configured. Enabled=$enabled"
        exit 1
    }
} catch {
    Write-Error "$logPrefix Error checking SHA384 configuration: $_"
    exit 1
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUbOLYO24/TG68MiD3PEJCw7t8
# neWggiB/MIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# FEQ8iIVWgD7o434ifkBuMkGXYi6DMA0GCSqGSIb3DQEBAQUABIGAF6YScKm01HiY
# WOTUz15ViPggRhcEeH6spYMCO8oS/Yr2ydHt2E5SzqF2MXD+DHlyc7Er4dkiGcmT
# 3qs2oxSP7RAS5ee+s8BmRuieBM/Ej1Ll3DuybxC3PF0hdpbc85CqCNdEZ2f4nDxq
# 0Eq31OcDVg/bnXR6YqTLO05Rba14WnuhggMgMIIDHAYJKoZIhvcNAQkGMYIDDTCC
...
# MTU1ODU5WjAvBgkqhkiG9w0BCQQxIgQgAhNPMdnDkqZxHfiBCbjA3gVt31BFX50N
# Gg/OWf9JSYUwDQYJKoZIhvcNAQEBBQAEggIAryO6wJn5wD3plRD+ohB7mRUdwLLD
# xZyoU8eMCGPjRVjR1SbTGrSnauATD85cESvjoEtoJx/yC7RNBsfKT0YfoiDGJ/pM
# 6FbWHh9itbbQ3H6YhEvDwG9pQCboxe02Vjy1Y7t0aSRgA6bUeZJbSglv6lTXTyms
# HOz8S39j5EZZ/iJ0tO/FAwbmatgx23+pnzVB56rUbG/K5aFxfLnfWnG6nDyN4qvc
# oslbRqcLSUqeiQu8eUpo4+VqE5NCP2nSfOsWz7fA2SQjP7yEwG1oMyGzffd510gq
# xUJmcFVJkShCcHQIpff5KlXBz9ERAp37hYsMgnAX43SMgr1IlVJZBCK1AsUwokb0
# 0w9bJy6VZ6lgG5PvomwiuXomVYzft7jvv6UedmBBsPwphm6DCcsDx5HMfByFz+mH
# OnSFeGCiU6WJmcjJPs/byh8+B62epVArONGFLraF214KQC3WhDLuvvGmHKm7w+6k
# ZcjiFAWf6JczFSeTbpIWYWRuSCEy+Xbn9ef+cezpI+Duth/pj8c9eEHjPs98eYHD
# kpiQ2GMQYafiHdyfBPlU0yR2LNCaHdjrmv3zIN8lQAksAoXgEEFJ23ZNjyLL89j6
# zTUluibPTbtOgWs/X5rK5vS9r4HVJn5MCHSVytNVG/SQUBoai0OrJPzBlCmbGxwF
# WRYi0E28VkN6fQE=
