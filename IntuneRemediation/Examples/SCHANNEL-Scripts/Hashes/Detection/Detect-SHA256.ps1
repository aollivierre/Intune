<#
.SYNOPSIS
    Detects if SHA256 hash algorithm is properly enabled
.DESCRIPTION
    Checks registry to verify SHA256 hash algorithm is explicitly enabled
    Used for Intune Proactive Remediations
.NOTES
    Author: Security Admin
    Version: 1.0
    Exit Code 0: SHA256 is properly configured and enabled
    Exit Code 1: SHA256 is improperly configured or disabled
#>

$hashName = "SHA256"
$hashPath = "HKLM:\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Hashes\$hashName"
$logPrefix = "[SHA256-Detection]"

try {
    # Check if the hash registry path exists
    if (-not (Test-Path $hashPath)) {
        Write-Output "$logPrefix Registry path not found. SHA256 not explicitly configured."
        exit 1
    }

    # Check Enabled property
    $enabled = (Get-ItemProperty -Path $hashPath -ErrorAction Stop).Enabled
    
    # Verify correct configuration (Enabled=0xffffffff or 1)
    if ($enabled -eq 0xffffffff -or $enabled -eq 1) {
        Write-Output "$logPrefix SHA256 is properly configured and enabled."
        exit 0
    } else {
        Write-Output "$logPrefix SHA256 is improperly configured. Enabled=$enabled"
        exit 1
    }
} catch {
    Write-Error "$logPrefix Error checking SHA256 configuration: $_"
    exit 1
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUdEdZePFFUM3tBD/8G5HWtjhU
# R4uggiB/MIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# FIbkLzXoSvAns4GKy1CMD3C1fRZBMA0GCSqGSIb3DQEBAQUABIGAW3C7+u+YDZF9
# sJkkZbWE/Hc3RfOvIE8Uf3lMr619NgplLCNNipOL5a71VRZenGu6i9y21UuvL/3N
# Rbjmfb6KlmTKK+e8u3I3kWhP8tJ5Rocfh7PnUfIbiUvNCP1tXZqg2df1uDvJ1B7z
# WtDHWHa1WUUYoAlHKWftODOhtI/pi6ehggMgMIIDHAYJKoZIhvcNAQkGMYIDDTCC
...
# MTU1ODU5WjAvBgkqhkiG9w0BCQQxIgQg/ngQ3jClUNR8/MNJVhwiyc6y/CYyA9am
# 1K8dPsgQaXcwDQYJKoZIhvcNAQEBBQAEggIAmgi2ROrKIwI2bQlMyWw/Q4H0WGTJ
# U+u46+eF6MqptbZwCVkSCydzKUuAKV8VDXRRejHh8x5voKjfTejg8+RfvnYM+fvg
# s0RRr5hs3ag544WQ8N9b8u1Xux3gBJLpm4riOmZ2jy8vTqXueIaTuZfgAnJ3ssPS
# Swzz8hf7I2FNGphj9REDXGRd5F8/c97YnxzWRA4s4/GKo/JxMRrIaV8jkRWenZbV
# 6hQZHn1XMqdhc0D20i69MurXT0KLi7iu56bfAft7RmYLfHetO/tUfBIJoUel8fQI
# fQTXi7knsUdlp+DuQl34GTnrqAoHpR9/wP6udjUpjLXyATZsJxgp8uHHhk+HEJnk
# 05MTs5GF3sxZuH9wXFhQFDW3Ngv/FhTrPQUfJG7tAw+Qa3vcMfSMdOd8FoLLxqfO
# Xgk/pkchmDrACvUNUT8MuGesXnjpM09B1hiqUg4gDP8xJeYiaWtN8pcJ9KNsPa1c
# SkXtRnCBHTt0h6LHPjpnmt2YQ1hj+eWgbeVeNQbNUKG/GKXhw4gspqNP3LMjsJC4
# KkPwoiK0XjyCL8tfJzeXdWQ+ErUivqh/x6KGWNISLZ0/pdr9S4e6RejK55x2otNH
# nzBqFTh4ujufhN5flPrugYmr3D/clnW3G9/b7P4zmKNi/OfIBzdjZ7gFy4EWKT5H
# Mevut+P8FZZ0T6U=
