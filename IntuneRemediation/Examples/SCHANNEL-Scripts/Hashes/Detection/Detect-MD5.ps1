<#
.SYNOPSIS
    Detects if MD5 hash algorithm is properly disabled
.DESCRIPTION
    Checks registry to verify MD5 hash algorithm is explicitly disabled
    Used for Intune Proactive Remediations
.NOTES
    Author: Security Admin
    Version: 1.0
    Exit Code 0: MD5 is properly configured and disabled
    Exit Code 1: MD5 is improperly configured or enabled
#>

$hashName = "MD5"
$hashPath = "HKLM:\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Hashes\$hashName"
$logPrefix = "[MD5-Detection]"

try {
    # Check if the hash registry path exists
    if (-not (Test-Path $hashPath)) {
        Write-Output "$logPrefix Registry path not found. MD5 not explicitly configured."
        exit 1
    }
    ...
    # Verify correct configuration (Enabled=0)
    if ($enabled -eq 0) {
        Write-Output "$logPrefix MD5 is properly configured and disabled."
        exit 0
    } else {
        Write-Output "$logPrefix MD5 is improperly configured. Enabled=$enabled"
        exit 1
    }
} catch {
    Write-Error "$logPrefix Error checking MD5 configuration: $_"
    exit 1
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUm7gVZ6FSjQee/MwrjojeyuNf
# Rd+ggiB/MIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# FKSP7Y+I0b6yRHvHEj/z9shzu3MhMA0GCSqGSIb3DQEBAQUABIGAXcwzM8o6T9gJ
# 0Q3sFbmkAPDkZdDT16pZQLKM0j8kabDBI7nv0ZISO2pce0OYhdJKfvLgR+uYdtkB
# dHe/98VeHCRsWzXGL0rqU6nzS56OHlhwX+StOFoY1VDKA1xGvM5FUXyOkhRgcXUg
# Vbxc0q5vIQJ05qVAWe8kpyvSxyA6B+OhggMgMIIDHAYJKoZIhvcNAQkGMYIDDTCC
...
# MTU1ODU5WjAvBgkqhkiG9w0BCQQxIgQgZ7Dv+zVwI9ZQRRfEOXxbmG1Yo0jjVcDi
# cASu5TuFWE4wDQYJKoZIhvcNAQEBBQAEggIAeQVv97ElJ9CNUxr7p6UBb+OrFGt3
# v9KmvYFvv2iECI+kwI4NDJSidsMk+HjdsHoduo2mZG3qPcco/2GzI3Egnif9LRoR
# zW0rEdMCyp37Z37w/9EUSS8ubYtZRb3M43svqhlV0tPfk02jakvRqiB5zvUjbCJn
# ZBv1ZpElA+NbLlr0suBhmAXfImqUw/im2VqexrLpaWm1PU80wsOctLymrQRjQ7PU
# 8oaIKg353oBVXRXdlE8incg59oKKA2HNkjLnkIoSsdV+e57KOYKuemdx07rML3QP
# SFQVJenVgApNogoEGNfbzeQ0sTv1PjKdt3gBceGmfsbWM+JnBq4ZY1wnIrJxkElI
# 1h9oI82sa9Ex5Yi/5bYNnMCdmnRrf6ZZOdYLRcXdDRMNig+2GOZpg/sjxhQMGhzW
# guf0Sd39d0nDqTLX0g4OM7SB0/TiOAFLkosU6P09DBPy3Zbc/nFD+UgXg7qwyTvx
# zEglm80TooBVJnA1/q3I9blg1XcdqW8tLtz1yYnGr64WEByGosv/IQli7WTiyXMD
# eXje9gySOzQ1i7LrojNvZG+zMax5CIOODEpiOw9WCqek8xiOGxOhYxF2d5w2UvSW
# UWxvIE+E318tJaFrkSGg7LyYY6sA+F2ozCbznfClhoB6j6ag29ZuoD488IHAJlS7
# hxe36SxyMgUHNZw=
