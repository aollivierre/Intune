<#
.SYNOPSIS
    Enables SHA hash algorithm
.DESCRIPTION
    Creates or modifies registry settings to enable SHA hash algorithm
...
$logPrefix = "[SHA-Remediation]"

try {
    # Create the hash path if it doesn't exist
    if (-not (Test-Path $hashPath)) {
        Write-Output "$logPrefix Creating registry path for SHA"
        New-Item -Path $hashPath -Force | Out-Null
    }
    
    # Enable the hash
    Write-Output "$logPrefix Setting Enabled=0xffffffff for SHA"
    Set-ItemProperty -Path $hashPath -Name "Enabled" -Value 0xffffffff -Type DWord -Force
    
    Write-Output "$logPrefix SHA has been successfully enabled."
    exit 0
} catch {
    Write-Error "$logPrefix Failed to enable SHA: $_"
    exit 1
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUfyatygDcXXEVCEoyQ3K/dqOo
# 33yggiB/MIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# FJjm7Ib3ed5TqkTEpfgcx843L+1UMA0GCSqGSIb3DQEBAQUABIGAtWv7KwKPbbz6
# viQ3DXMJIM5M6OUHuAD5aiiPiW/HRm38ywniyt1pnV7k/tJudPIOBJEJupYbOI5N
# Bq3hnvHr5T/aaVS6NQ9sc7Us4o5bnSoLQ8+cmil5/qyHVR4BUGn1zTB3E+TT7+ei
# rL7pQqGi8RXeuYSNp5PrRnOvNpfTHwahggMgMIIDHAYJKoZIhvcNAQkGMYIDDTCC
...
# MTU1OTAwWjAvBgkqhkiG9w0BCQQxIgQgoPF5JDZOwpsEhU4W9t2f578Q8uBWF4D6
# gwyqusm2+f8wDQYJKoZIhvcNAQEBBQAEggIARMHH7c6trrArOgH8PAbaLHe2Nn5H
# f3jmZeYLLCEsd8NTVwNcRXWxC3P2jXCto4g+uqCSFOo3L4PGY1Eoy3xbn7qCAW2W
# 2KUw6TNdJgJZHA3d7O5ueawczLzgS9/Q+k31KdqrhyIyQiyx5fefi+xNl+cMHz5m
# iO/lWxO/8Xww45J5h90GoRWBWMPlUlvtUKrTeCHjIBcGW88FDZhCL5YbwekWodQY
# DDU5FB9O/JdYO0Seu5tVogrtC68L9wnD65NWFLLC/bW7wsxRQmsk/k9KUgrPSpDg
# ku73Cji3Ru31hom10m9YTq8hk86kVtiz74gfw/mIM+pfPmrAQVpekgU1R2tnwlQC
# yCq7+LmllciHboljaURJUjzCdl74GGOalVzDK0IViUS/bupUQNJYogOS3OnKXdRZ
# /NU8kuifk79toe9XKgH6AwTJE9ZWgNMiaS6pnjXswLawmm4y+xEFlHrVPG94wU55
# uukDjnJ7I1YiN44vxB7Ff6djKmihMZMgOQtKhW9IVlPGcug3MgLCBYr3jv5w624i
# RsN6iaupObugsAnkvEPU5Bhn2S1itx7+jL8lbA01ixfi9AMQmNEfgsQRsmNeD7Ku
# 1xnE42iyFsW32oAVC7Ro5Q7Y7IDMZmSMgizyCuiJrt6ynrVnKi0dTxyRAe9F9pVu
# gsARYpVs8R2GavQ=
