<#
.SYNOPSIS
    Enables SHA256 hash algorithm
.DESCRIPTION
    Creates or modifies registry settings to enable SHA256 hash algorithm
...
$logPrefix = "[SHA256-Remediation]"

try {
    # Create the hash path if it doesn't exist
    if (-not (Test-Path $hashPath)) {
        Write-Output "$logPrefix Creating registry path for SHA256"
        New-Item -Path $hashPath -Force | Out-Null
    }
    
    # Enable the hash
    Write-Output "$logPrefix Setting Enabled=0xffffffff for SHA256"
    Set-ItemProperty -Path $hashPath -Name "Enabled" -Value 0xffffffff -Type DWord -Force
    
    Write-Output "$logPrefix SHA256 has been successfully enabled."
    exit 0
} catch {
    Write-Error "$logPrefix Failed to enable SHA256: $_"
    exit 1
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUCY9qX9elSEJJck8AxpItcqpI
# T1WggiB/MIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# FN/VsqMdZXAcHxWFm2OHriEdjOfTMA0GCSqGSIb3DQEBAQUABIGAJrFOnzaTjYey
# 7FzOj9YgDCfM8/kW2L0d37dhccXiSFuBCTK9xRUDkyrmW1jfSfnhiJEUWFDHo2aT
# bAGa4YO1Zgh2Pclt5J7yEuHZP9OaIMp5jwbIX7oi/EnFifKtt0UjDlUZ60adBMU3
# ciNL46AoDan0Ycsh6TcuyFRZ+dKOBKahggMgMIIDHAYJKoZIhvcNAQkGMYIDDTCC
...
# MTU1OTAwWjAvBgkqhkiG9w0BCQQxIgQgYoOoklQOt0iTUybpJTDe5N6iqkxsWNfh
# msP+5LIMlDcwDQYJKoZIhvcNAQEBBQAEggIAJ6PU0C6tD8H87hGd1VpKc43DJWrr
# XQrKbj9vrwK0IcPVPbW+oF1h/894CSTLLgmOikvV8NQRg0uYwYRRa2g2wkVwM+Jr
# IIqC9DMJDDmfpUY2JkhAACXN85QCWK2HfBjXFKWLHRiZOLDq7j5Rv0tdPRnGVUsS
# W6Rw+FOD4rOzIqCpqSM4vdB5l/lBh5WjADLM29rFFlvi6Kq+VAQ3SoeOFIh2/CK/
# 3wIIgFHhw9bWDIms6DLWDa8Gpo1+f3ymJ1kAPqEhu6GUIROlNb16B6n4WzdSCBya
# 8N535gGBGCrgO0z7G1bAT2GeVyPs2Ad4e6BELEkSODfnHGv7wyfpElTHxWIHaDEi
# TLOLZeGyBzokEaqkce5aQdIhvJhIjsj7PhejzAbpDYmrGYkxmRpUURY+jWF+7SvY
# ITc31lFFX6ZwXR0JenYdW2ES4qkSxKeuvlpqEDSou43oaXwIwPoStoGoMAPtebhq
# 1DC4npQ+RTVG5qdb5bK/snPl95Mm+WzK9aAg4Fs65uQBstZ2HzFqxZZL4ll/uUoI
# U04ytm0qJyEv/jXpXtEHDYR6jW5c1S923Il2uXcCe3bqM4V7vWLRB8zojza5TQV2
# OmLV1tz43Nm+sH1fPHG/yXozgTc7U2VxvEeoTb38Yo6enUTAgC772bzeVp/cYyPb
# 7emGC4J4uckdcUA=
