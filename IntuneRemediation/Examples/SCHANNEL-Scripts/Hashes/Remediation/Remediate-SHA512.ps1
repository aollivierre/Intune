<#
.SYNOPSIS
    Enables SHA512 hash algorithm
.DESCRIPTION
    Creates or modifies registry settings to enable SHA512 hash algorithm
...
$logPrefix = "[SHA512-Remediation]"

try {
    # Create the hash path if it doesn't exist
    if (-not (Test-Path $hashPath)) {
        Write-Output "$logPrefix Creating registry path for SHA512"
        New-Item -Path $hashPath -Force | Out-Null
    }
    
    # Enable the hash
    Write-Output "$logPrefix Setting Enabled=0xffffffff for SHA512"
    Set-ItemProperty -Path $hashPath -Name "Enabled" -Value 0xffffffff -Type DWord -Force
    
    Write-Output "$logPrefix SHA512 has been successfully enabled."
    exit 0
} catch {
    Write-Error "$logPrefix Failed to enable SHA512: $_"
    exit 1
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU6oHKvKt0Xp7NkfgjsbiJFLE+
# oSGggiB/MIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# FL4YdWD6JrMvKQFIophylANsIkw7MA0GCSqGSIb3DQEBAQUABIGAYtsX4un4/sys
# KpFUyLUdn+TOyijv60sN/NIFG5sRm2b3dFEhZ8pqqrvqnaGgHII9Fgw9FqphayRL
# Egar/sfKCAiO9HSgW4Nrar9GznMwFn5eg70V97DJDCBQ61y0vGXNN04B75hw+a0p
# YQzwLsMt4cp9hbWLfIUECVnCSNb628ShggMgMIIDHAYJKoZIhvcNAQkGMYIDDTCC
...
# MTU1OTAwWjAvBgkqhkiG9w0BCQQxIgQgSHoE7CyFQ6dE8r+EddHAdBZ8AxVRJJtd
# B4zLMNwNB44wDQYJKoZIhvcNAQEBBQAEggIAad55mP8+fIGKhLPWxa53yrWOVbh6
# /QdbUd81qndJ4EnafSaMOv/aN6hU5nyW7oqbxI5Lv9nhwFmaeZF3eZLqk/o7P3DM
# L08MSptVENwVwPohA1Hk30j9OykGoBNB6x9Nb7yxPMK7VcePe7aKVG1z0nDx2wYL
# vilTw2AW63xJ3/5QyksI2U8OWkW+uJfOUS/GxnAalHYbuW/caJRhib/zKpnmUjZd
# DiFqffi12abAEKYmd5aujUDs3VxfhtqXTOhsoEvFlb4Fe3wobmQmEfo0CuQf0qT/
# mvIq9uq58MBHS3umy5GIzv6M9D1c1LFVSwWTUOIvd5hDByGQv5GM1NC0Aey+/dIm
# XFgqImW7WSjcvdyxR/gLF14Lqn0GYGFgY3GLeHj58W1/wsJzizO+RtR2TSY6Qh+J
# M9Nkm2QZQUdpcONyMYHFUtuSKDtPfp01cUKHpRA6W0wwanj8uw9asLLB04Q7bumi
# dj4O7NCrM9+aSTWSvPHkGUPrb+/SoDv+UquPK2P9V4AuPKI7kyTxA/5NcSO2Qjxp
# p+/uX0xRUFgkAXs0n6hRUQiGkeuoV+3qDaFs05jdXft1iKrysGrOr5uqLmYYU1iT
# McMFxfIVy5mzqiAVj03kSYWjdFXyfcV1a20+7gkHxmM5wxT9imcceos/ITao13wV
# XxqDi0JLTNNztsU=
