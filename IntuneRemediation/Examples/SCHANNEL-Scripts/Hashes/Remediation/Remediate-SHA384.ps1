<#
.SYNOPSIS
    Enables SHA384 hash algorithm
.DESCRIPTION
    Creates or modifies registry settings to enable SHA384 hash algorithm
...
$logPrefix = "[SHA384-Remediation]"

try {
    # Create the hash path if it doesn't exist
    if (-not (Test-Path $hashPath)) {
        Write-Output "$logPrefix Creating registry path for SHA384"
        New-Item -Path $hashPath -Force | Out-Null
    }
    
    # Enable the hash
    Write-Output "$logPrefix Setting Enabled=0xffffffff for SHA384"
    Set-ItemProperty -Path $hashPath -Name "Enabled" -Value 0xffffffff -Type DWord -Force
    
    Write-Output "$logPrefix SHA384 has been successfully enabled."
    exit 0
} catch {
    Write-Error "$logPrefix Failed to enable SHA384: $_"
    exit 1
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUC6A8XyuEe2VOQd+rplJWi/ig
# HiiggiB/MIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# FKfQCFBTkED4FxxrEpjfIpdqYJEcMA0GCSqGSIb3DQEBAQUABIGAjQv/AuxRdIYS
# mIbPSDjmJponC3lr6h9ABeC6dIyfelBZ2e6hmNhuhSVYYmLplfxtQmfQxGA2QrKZ
# +7WSIuhWy1/2QdwH2bmu/9WgAvBosHCHj1LknssqVh86SBU8wQAIVYp3SE1UgAH6
# lNKfm+AzORlqVxzfn9JMtsOH25WSaKShggMgMIIDHAYJKoZIhvcNAQkGMYIDDTCC
...
# MTU1OTAwWjAvBgkqhkiG9w0BCQQxIgQg+6fcCQ4uRn2j20h+8q0jz0c2cPhi7gcy
# PFuvawhONmowDQYJKoZIhvcNAQEBBQAEggIAV6tPUeT2nBSgO5ptbrDhPGgfPkqs
# 7x7fpk4WwSSHM7l7XGnT8Nh4PuOSpaUKAzQJgXkJRr4nZUtU2RgOT8aJD0/hSzzE
# wiI0jmlNgNtdfmAJtu9rETs+EJvGkYkIV0wwAX2BhFAJMsNjA611A+BBOaKXztdh
# QqfByuN1p8etVR9WZ5tHOgCPg3JLyVv4ER2XPkL8y8OGlAnILtq5C10cFBNy3HTf
# 3QbUcAEE3odaJ+et0cQL6JSXztnG4HhSlLm+PMhuYpp1Qy0d+I+Dvj1ZKXbYNj3b
# EdtVwV1i66FHihFA/F2Wjm94tVXzq6urlxb0H0o2kYjPRkc4xBRY1glk4P2Mmm3p
# ZmIplo5nadPD4fbRK/uqSSV1tRGVKyMnr3pxZ6xfHd/iVgSG22wjLs2an3vs/THT
# kdqxReVaAVa2ZUnOpEy9uxfnELSfWvXUGfLljsmq46nqTK9dnJkFA8sj8EdzqMnU
# YeIhePHlTHFqU0KrgKe6rZEEMyHzCTriWIGuEhUor66sdQIl1jMptgeILSecfFcX
# UW5HZL4GodICvAYQIOQ4TVmdqJxuvJTpVxuWTRyxhPOnsBx2mLESfqBU01IOjlcZ
# 3442mMyKDT+Fu9PFyA4vtM1pzoSb9lUOMqsbJ/Yp9fuLRAxdQI0mC8JgwTuXj7/O
# vCgRrOonQwgHq10=
