<#
.SYNOPSIS
    Disables MD5 hash algorithm
.DESCRIPTION
    Creates or modifies registry settings to disable MD5 hash algorithm
...
$logPrefix = "[MD5-Remediation]"

try {
    # Create the hash path if it doesn't exist
    if (-not (Test-Path $hashPath)) {
        Write-Output "$logPrefix Creating registry path for MD5"
        New-Item -Path $hashPath -Force | Out-Null
    }
    
    # Disable the hash
    Write-Output "$logPrefix Setting Enabled=0 for MD5"
    Set-ItemProperty -Path $hashPath -Name "Enabled" -Value 0 -Type DWord -Force
    
    Write-Output "$logPrefix MD5 has been successfully disabled."
    exit 0
} catch {
    Write-Error "$logPrefix Failed to disable MD5: $_"
    exit 1
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUQAgJhaZeuSWWrZv46nhTvh3q
# gTyggiB/MIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# FN/gyt2TVQ74ADY1Jq+wU/8jbdezMA0GCSqGSIb3DQEBAQUABIGAjX81jQEgLAXT
# /biOSWmLTjRRlgpA6z55jwtJoOciTc3lLv/tbI2/UU9cr1nUMrU0G3fZ/85Hcnzf
# Ha0DyejhxYZJrdsQMaHKlJF0PlvK/LaS6Yul0VJDKFvy0dw3927i6tQgE6rRdmey
# l6l7NvVjia/KuEpPcAYOKQccp4Q9MUShggMgMIIDHAYJKoZIhvcNAQkGMYIDDTCC
...
# MTU1ODU5WjAvBgkqhkiG9w0BCQQxIgQg4pRiEABjAhw3LS9ORcXQ3EVlNURDJT2K
# qbUWp88RErkwDQYJKoZIhvcNAQEBBQAEggIAnBEz0dInh5uXxVbzBYgo6tKiEMOm
# LulQ5Ie5ODcQvXiJ1wsAaBDZsi0wJgR7rPy+XVN/5cHkICIEzOUL1TsP3XzZbUd8
# hMsF3oLlrWBnb2wVxX2chsTN5ppQqXklq+f6Ru0R6yiDQ+taB7g0z82A9lgLqfw1
# R0LIimLE86cTiJ5+0L1K9yMUSn+kaUle0J+BiivOnzMZ4O6hVcl4BKqeoEplitJy
# SvF8Y0blDefRCI77umkJjuWIN09S3hyn0PhA+7qucB1LBkVswL1g0jHgiPppOunf
# xRzVy7Gs6rR5YbBJIkgLUQNbpOc/NyYGJXEEajBUyko3J11eGAhR3Hb7bC5NKqPk
# 53lYmWT0wRC/+/SM1m/EgjJ/85SoT4/gx0vTefQ6A79Fq4JjLnnObkLYwdxGkJUQ
# D5ofGw81dqMYax/7+9PKWbOph1oPMao8Thp/P6hMvzMkrXt+ZVQMsXdNvBLXBoPM
# GFsZa+i3U7GUO24uYzAJufieiCEIKx7KFrjOpF0jSpUUYMdbunOT+LNPC0yeVqmq
# xyxEKVWTdecgjzKhjHHQFTVHSqeGLSCl5xwNkrOJCm2ygTVlOokdA1g7aKTyT9Ez
# Sps03xYQ4JwUVJS1gCPMnhXohaeSgkTXZo2cVxmjYD/wPKy6QnhPQg7cAYvdAGS1
# 1BAnNSS44VP4omA=
