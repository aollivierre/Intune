<#
.SYNOPSIS
    Disables DES 56/56 cipher
.DESCRIPTION
    Creates or modifies registry settings to disable DES 56/56 cipher
...
$logPrefix = "[DES5656-Remediation]"

try {
    # Create the cipher path if it doesn't exist
    if (-not (Test-Path $cipherPath)) {
        Write-Output "$logPrefix Creating registry path for DES 56/56"
        New-Item -Path $cipherPath -Force | Out-Null
    }
    
    # Disable the cipher
    Write-Output "$logPrefix Setting Enabled=0 for DES 56/56"
    Set-ItemProperty -Path $cipherPath -Name "Enabled" -Value 0 -Type DWord -Force
    
    Write-Output "$logPrefix DES 56/56 has been successfully disabled."
    exit 0
} catch {
    Write-Error "$logPrefix Failed to disable DES 56/56: $_"
    exit 1
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUtb6LLawTONdX4xqNMs9QqlBd
# xM6ggiB/MIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# FMq+eQjNHmbLiz+gVyIRzVpSJKLwMA0GCSqGSIb3DQEBAQUABIGAhPNejk+h2eNk
# fEAKJEj1xpMvB/fWNpaFxzmR7lYXUE1p0lIZWJrB1vst3jVVRxkXTdCQJ8AR9va1
# QkyR6zemRWMANmqxKwF6OYD1dce1cwsby75fANmXeUCOqrgOuVRsU8xtXIEDswkL
# xhkZbqricIHpOU/BGUITaQnPTTvrIoChggMgMIIDHAYJKoZIhvcNAQkGMYIDDTCC
...
# MTU1ODU3WjAvBgkqhkiG9w0BCQQxIgQgb0hqqpXW8JL9UQp2iDmmE7TglXq8Rfh1
# ugs1sVu0wHIwDQYJKoZIhvcNAQEBBQAEggIAtqE2i9ePqMT2ZExFmZanzuNuRvak
# /NlT1sKpvu7YwjFEMVZ+3YfnMZD5DfBw+Mi80rnCWcpbhoNsN9khMgqB6Ggs9v0I
# Z5rJDK+j5BB2ZFrvZ5qbDeZtp8v37YDq+3u8imzvIi++j1ovL8aBTh1jIFZz2PzD
# luJ/EvIUQmQN4wJ4wU2GlhE/xEAcReqxVtzL6wcCJKgTadP3yrk6cnZ1+OVaQgaQ
# xrymkOdUZGF46GKO7RDOGbxQUBKOI6Mw2Vt4u/Qt13FROeOb999zhU3WBVKwZwLV
# m/oZBHvAMNPLH1Q2eUxd3JWVj1r2Kk7C9PWYArDzaobJV3+0TCQMDyPYnxhQDQqL
# 6kKMtVfXmlIOJxMOcz/7BOefw+86jU6p/ebNDJ49G6oIX7D4sBgvQRwWp56wB+Io
# q2CM93isTlkz7cVHKR6kzle7gfK/y31OQzB1X4G7KpIQm3PApD2op6XsYQT3zSQd
# wh/Nvy37cExvBisRkOvhEH41Ems1OKpy6VFOatvgOveANJUmDELw/IVAb77uc9gK
# ZILiZZytochtBooVFId/4r9Xba0Z+zvcvE8mYExOrhgEdDrB2V2QIdDypEYhWIvi
# x0ksFPzs685jn/18tT1EjMny/rKbgLq4OP1pAPHllGHLpiSJbckExk0XFt32JK9J
# H46xMa2f1LEr8qk=
