<#
.SYNOPSIS
    Enables AES 256/256 cipher
.DESCRIPTION
    Creates or modifies registry settings to enable AES 256/256 cipher
...
$logPrefix = "[AES256256-Remediation]"

try {
    # Create the cipher path if it doesn't exist
    if (-not (Test-Path $cipherPath)) {
        Write-Output "$logPrefix Creating registry path for AES 256/256"
        New-Item -Path $cipherPath -Force | Out-Null
    }
    
    # Enable the cipher
    Write-Output "$logPrefix Setting Enabled=0xffffffff for AES 256/256"
    Set-ItemProperty -Path $cipherPath -Name "Enabled" -Value 0xffffffff -Type DWord -Force
    
    Write-Output "$logPrefix AES 256/256 has been successfully enabled."
    exit 0
} catch {
    Write-Error "$logPrefix Failed to enable AES 256/256: $_"
    exit 1
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUcX+4l3iy7CumChyQZUa4Yptt
# kIGggiB/MIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# FKPCCQnVZCRwD95qok5ssh+nkdx6MA0GCSqGSIb3DQEBAQUABIGAQwVnBlxtXUG/
# hQjpqqU5iG3hEEiMp16DUvs/luK5G+7e3phFf/PwyANg39DxJ/M6ktfqHzpNmxSK
# tHJOgtE/Y7y9sOkWIobsAci8luO2S1pO8CXubgbgjemEESugAa3mldQuH1yJvu3q
# H08t5ysmXyIlGEQyY9EgzFaIlIO/sZahggMgMIIDHAYJKoZIhvcNAQkGMYIDDTCC
...
# MTU1ODU3WjAvBgkqhkiG9w0BCQQxIgQg5UMCrN47lHAJToZTQqv+NoTUv5pfEZW/
# oCnV7o6wFE8wDQYJKoZIhvcNAQEBBQAEggIAJO55dcmzK89G/aC+ryli72oQVpWk
# Od00jPO7XLI6CV9KtS5bupH72YetUcrBTT9ojrAtZMnyazHWhp0WP481hjBd3GSd
# sj/84csx4IypizP6XWxwphFBWsnUf33ovj8btQtOeVrTid4QmA7oRXJ4Z/gfFjQi
# YudPalvF0zMd8F7xWx+wBwNkMWpWyFV8KXtveZYki8i4/Hvnuu1THgLTxQUE3WwO
# d9jb3NgSukbSL4vDNJL4kLCJEvuE04KojOUroJCgD+v4iNlGVODpG+b8AQqve9dF
# oi1QlGNRbfbjkgw+BbVKMFpDnx7MeSEmarsKVeDpHomHleGAnUUVIlOlGUYJaEqL
# XXerhUEF0MJG6xBMr76906ylzpJF+kjeHh9+9+GDAVPuTe7hzDtUhi5Q8m2gHUIq
# XQcJgozDK86Z8zAt2wkpS79NvdwuMhoMn4AdBmAnRq5sokXR8YWjruIpDKT8w8lj
# raHwBz7uOLMgWQkgRwxCEuplkWEQGGU01vips1UuIvdF1Hv16nNjHGncRv9+QgWO
# 451DEjiCnRAoI5K51VmRdZTP05udaXo8Acz6wOSdrd5Nc7w0BoEy3O/wdNl5rsLc
# MSgYKGxudo7RXgy640Icyo993iNO41SeP7CpMJ5Wkow3r3aaSe+omILZqoXYdFW9
# vUtwXYHdxp2fauk=
