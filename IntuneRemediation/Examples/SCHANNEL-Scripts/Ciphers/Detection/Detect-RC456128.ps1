<#
.SYNOPSIS
    Detects if RC4 56/128 cipher is properly disabled
.DESCRIPTION
    Checks registry to verify RC4 56/128 cipher is explicitly disabled
    Used for Intune Proactive Remediations
.NOTES
    Author: Security Admin
    Version: 1.0
    Exit Code 0: RC4 56/128 is properly configured and disabled
    Exit Code 1: RC4 56/128 is improperly configured or enabled
#>

$cipherName = "RC4 56/128"
$cipherPath = "HKLM:\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Ciphers\$cipherName"
$logPrefix = "[RC456128-Detection]"

try {
    # Check if the cipher registry path exists
    if (-not (Test-Path $cipherPath)) {
        Write-Output "$logPrefix Registry path not found. RC4 56/128 not explicitly configured."
        exit 1
    }
    ...
    if ($enabled -eq 0) {
        Write-Output "$logPrefix RC4 56/128 is properly configured and disabled."
        exit 0
    } else {
        Write-Output "$logPrefix RC4 56/128 is improperly configured. Enabled=$enabled"
        exit 1
    }
} catch {
    Write-Error "$logPrefix Error checking RC4 56/128 configuration: $_"
    exit 1
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUyuEsIW4obKktaN9J2JcT0y3z
# KVaggiB/MIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# FJheBqUez/Q8XSiYYoLy9Fc2Ni+dMA0GCSqGSIb3DQEBAQUABIGALtQfrbSnREQS
# wmWtFBlBoRI+Xqi2IZo37GSaYpuCwzZqewy9fDgLMiUNokcJRBSNZc8vqETqZrOE
# Gf4uc/opxCLxtbDGRz8ftqfAdWtq3pU0DNBeiJG+Jw9/88xHhPz0BZiEOOO13y8A
# UU2m5ZtZsxVJp+/glU+TpBxC6LE5Np+hggMgMIIDHAYJKoZIhvcNAQkGMYIDDTCC
...
# MTU1ODU2WjAvBgkqhkiG9w0BCQQxIgQgA9dWRafyxQX0FcCkHkaF2+LUIy48RuOH
# YQFUNqq0lUkwDQYJKoZIhvcNAQEBBQAEggIATPKiBHkMEsCRnuuxa9db3FiB4VM7
# oJqZgnDwySAGa8izNy2qbgx1rydFe+W35O8tNWHmWYcCBSnOZSoI3vZhaG/Xi3An
# am4SjrFzhKDKolA6EF2TeZAGm0Cqzw3eJCqS0EYbuTkRtzBX9I9gG95tsR+l49oU
# 8qUKlXDnVw8zHkt9tbV53LiXzOmG/qq2wtkaB5qhxC6kBYLGxU4qnYQFA64RT36d
# 0FrrR3GTiPJ6uoiGjziir/q5L+K7Cy18wOqV/EARaIv8tjTIge7CCZx1pC+ZPHjJ
# 8SgirLmWMRAa35IQ1vc/e/lv2TZw2dF0/kOjvpW9LuDOw8xs8hJ1kMdqv6JV6Fpk
# vt4NHSR+XGU/w4EEbB8eacfmUZmxYCiKSPxthrIh+ZwY2G41ov/uFh+xVT7p46rh
# ZFxLCU8cyq3LeGp5hUToitzVSiYhVQYXmScAtqNFO6AwyNauefL39XgT28xV6jNk
# lv61AEr4VnIiH50j3wxHzaERZW60J3EvxmW3KM+vXXWWNCAZOjQggr/N2ZCmwiKb
# w1o40agmckZqU226E2KVx9qHyqYZFfyhXS1qU8P6t83GUWHEiazvSqxbLeEkPxri
# uNFxdupQ5HVsHSIs3iZfHDLFA907M7uT+agZ2YdJUx0+wBRAuKs71K/d96IdpmiP
# qSni52AmbxFm2dU=
