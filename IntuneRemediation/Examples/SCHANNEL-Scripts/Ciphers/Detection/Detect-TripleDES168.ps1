<#
.SYNOPSIS
    Detects if Triple DES 168 cipher is properly enabled
.DESCRIPTION
    Checks registry to verify Triple DES 168 cipher is explicitly enabled
    Used for Intune Proactive Remediations
.NOTES
    Author: Security Admin
    Version: 1.0
    Exit Code 0: Triple DES 168 is properly configured and enabled
    Exit Code 1: Triple DES 168 is improperly configured or disabled
#>

$cipherName = "Triple DES 168"
$cipherPath = "HKLM:\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Ciphers\$cipherName"
$logPrefix = "[TripleDES168-Detection]"

try {
    # Check if the cipher registry path exists
    if (-not (Test-Path $cipherPath)) {
        Write-Output "$logPrefix Registry path not found. Triple DES 168 not explicitly configured."
        exit 1
    }

    # Check Enabled property
    $enabled = (Get-ItemProperty -Path $cipherPath -ErrorAction Stop).Enabled
    
    # Verify correct configuration (Enabled=1)
    if ($enabled -eq 0xffffffff -or $enabled -eq 1) {
        Write-Output "$logPrefix Triple DES 168 is properly configured and enabled."
        exit 0
    } else {
        Write-Output "$logPrefix Triple DES 168 is improperly configured. Enabled=$enabled"
        exit 1
    }
} catch {
    Write-Error "$logPrefix Error checking Triple DES 168 configuration: $_"
    exit 1
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU7dvDDZ8K0Hmcsk55p5hvEMTO
# eqmggiB/MIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# FKUa1rZyqwWa1J0ByYCc7CajVptzMA0GCSqGSIb3DQEBAQUABIGAbi5XxZJlhIj5
# 9qvZZR/KuqCboRK6iZ97axY8jgPeRaWcRg15O6Mesnc6bEU46p5y9ySSVZDHLZbX
# Px/HR6dkG4qh/UFsjZq4CvD0hjZWf2wvPebAEBvYvgSvBk1UvlMzdvknHfh+dHs6
# CfEYFFkW/rv7Ggbr4bC0LQBxoEyM29ehggMgMIIDHAYJKoZIhvcNAQkGMYIDDTCC
...
# MTU1ODU3WjAvBgkqhkiG9w0BCQQxIgQgW3zf+1pgUvWUYEaWgy4M9phZWKy0sYBU
# 2y3pjoU9u1IwDQYJKoZIhvcNAQEBBQAEggIAmzaruOdd0DUhsN7xKeuXaA4U2tqi
# opNtVMxVAmGFIi7yjWYEVXLvLNKBHGUPQGGg2oyMDYOcRWimpMIsbsaVU85g5Rmg
# qFjdf7Pw2OiSFkyHKUwgX9nVjoqlZ/L5xD83XL9yU8J44ntkSRnWZqraiZKu+eYI
# OET6L+5F825cSEPaq1H1BwQY0qREKR0LKBDBAbWMcuKXVwJuyRQE63x0Fu3PXslA
# b/JksUXwGKU6fwN4ysLRkVjOdRUpk4FtiEbTfuBauMXJVhNZ9jaMxHj1eRpvhmrY
# Q0+uUZk13jemwzzJDnbnzhc9Lve6dOBao2DYMPDhfvxBE+vq26ip61791PdF10pm
# yBvyCHqmUMoXWDHjtPBPKsDKk7ofivvbhUYMgie67zgqGPETEJfyov8KVdNpFMWt
# 5tHZnX+3mHitCZh6VNwsfGQ5hoRGBSOWVcNXAV1e/ectZ67iFNVFWv54RKz2L+TW
# S4yTov2fotXCVwd4nXMaXmWHmTBBw/o2LnaWMuD7KNsQSz+QbWb7czAtXgLsU6hP
# AuKKmq8htXkpc2GHBewiIG0AJOZxVlRaXOf1te9MRwgDwg6BZ3yyVh/oDJh4eHQw
# y5Zc0zHHo4bvPMjM4Nak5TPxgO8e+rLWYQLYpxJgsbmGl6CyTnuIN0+9KF75Dxf2
# eLtsen8Qa7q2/4w=
