function New-IntuneRemediationScript {
    ...
    .PARAMETER Publisher
        The publisher of the remediation script package. Default is "Abdullah Ollivierre".
    ...
    .PARAMETER EnforceSignatureCheck
        If specified as $false, disables script signature checking. Default is $true (enabled).
    ...
    .EXAMPLE
        ...
        New-IntuneRemediationScript -DisplayName "Fix Service State" -Description "Checks and fixes critical service state" -DetectionScriptContent $detection -RemediationScriptContent $remediation -RunAsAccount "System"
        
    .NOTES
        Requires a connection to Microsoft Graph using Connect-IntuneWithToken.
        By default, script signature check is enforced for security.
    #>
    [CmdletBinding()]
    param (
        ...
        [Parameter(Mandatory = $false)]
        [string]$Publisher = "Abdullah Ollivierre",
        ...
        [Parameter(Mandatory = $false)]
        [switch]$EnforceSignatureCheck = $true,
    ...
    try {
        ...
        try {
            ...
            if ($EnforceSignatureCheck) {
                Write-Host "Script signature verification is ENABLED for this deployment." -ForegroundColor Green
            } else {
                Write-Host "Warning: Script signature verification is DISABLED for this deployment." -ForegroundColor Yellow
            }
            return $Response
        }
        catch {
