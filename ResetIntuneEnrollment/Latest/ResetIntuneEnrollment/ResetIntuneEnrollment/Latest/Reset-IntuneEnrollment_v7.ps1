        ...
        $scriptBlock = {
            ...
            try {
                ...
                foreach ($folder in $subfolders) {
                    Write-Host "Folder Name: $($folder.Name)" -ForegroundColor Green
                    Write-Host "Folder Path: $($folder.Path)" -ForegroundColor Green
                    Write-Host "-----------------------------"
                ...
                if (![string]::IsNullOrEmpty($EnrollmentGUID)) {
                    ...
                    try {
                        ...
                        Write-Host "Task removed successfully from path: $taskPath2" -ForegroundColor Green
                    }
                    catch {
                    ...
                    #TODO Start Intune Management Extension Agent service and CCM Agent Services
                    ...
                    # Sleep
                    #TODO Sleeping for 30 seconds
                    Write-Host "Waiting for 30 seconds prior to running DeviceEnroller"
                    Start-Sleep -Seconds 30

                    # TODO Start re-enrollment process
                    # TODO Calling DeviceEnroller.exe (AS SYSTEM)
                    Write-Host "Calling: DeviceEnroller.exe /C /AutoenrollMDM"
                    if ($asSystem) {

                        #TODO Calling Invoke-AsSystem
                        ...
                        #TODO Calling DeviceEnroller.exe (IF not as SYSTEM )
    ...
    function Reset-HybridADJoin {
        ...
        $param = @{
            scriptblock  = {
                ...
                #TODO join computer to Azure again
