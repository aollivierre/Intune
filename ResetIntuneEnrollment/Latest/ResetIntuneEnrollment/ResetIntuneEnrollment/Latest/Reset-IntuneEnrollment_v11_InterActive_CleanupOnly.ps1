...
foreach ($folder in $subfolders) {
    ...
    if (![string]::IsNullOrEmpty($EnrollmentGUID)) {
        ...
        try {
            ...
            Write-Host "Parent task folder for GUID - $([string]$EnrollmentGUID) removed successfully" -ForegroundColor Green
        }
        catch {
            Write-Host "Error: $($_.Exception.Message)" -ForegroundColor Red
        }
        ...
        foreach ($Key in $RegistryKeys) {
            ...
            if (Test-Path -Path $Key) {
                ...
                Write-Host "Error: unable to obtain enrollment GUID $EnrollmentGUID value from $key" -ForegroundColor Red
                Write-Host "Error: $($_.Exception.Message)" -ForegroundColor Red
            }
        }
