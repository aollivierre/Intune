...
#     # Output the values to the console
    ...
    # Write-Host "CSVFilePath: $CSVFilePath"
# } else {
#     Write-Host "Initialization failed, variables not set."
# }
...
# Write-Host 'writing file name' -ForegroundColor green
...
function Install-RequiredModules {
    ...
    if (!(Get-WindowsCapability -Name Rsat.ActiveDirectory.DS-LDS.Tools~~~~0.0.1.0 -Online | Where-Object { $_.State -eq "Installed" })) {
        ...
        # Write-Host "Rsat.ActiveDirectory.DS-LDS.Tools~~~~0.0.1.0 is already installed." -ForegroundColor Green
...
function DeployOneTrace {
    ...
    # $deployApplicationPath = Join-Path -Path "$scriptPath2\Install-OneTrace\PSAppDeployToolkit_v3.9.2\Toolkit" -ChildPath "Deploy-Application.exe"
    ...
    # Log start time
    # $startTime = Get-Date
    # Write-Host "[$startTime] Starting Deployment of : $ProgramName using PSADT" -ForegroundColor $infoColor
    ...
    try {
        ...
        # $endTime = Get-Date
        # Write-Host "[$endTime] Error executing Deploy-Application.ps1: $_" -ForegroundColor $errorColor
    ...
    # Log end time
    # $endTime = Get-Date
    # Write-Host "[$endTime] Finished Deployment of : $ProgramName using PSADT" -ForegroundColor $infoColor
...
function DeployIntuneManagementExtension {
    ...
    # $deployApplicationPath = Join-Path -Path "$scriptPath2\Install-IntuneDeployIntuneManagementExtension\PSAppDeployToolkit_v3.9.2\Toolkit" -ChildPath "Deploy-Application.exe"
    $deployApplicationPath = Join-Path -Path "$scriptPath\Install-$ProgramName\PSAppDeployToolkit_v3.9.2\Toolkit" -ChildPath "Deploy-Application.ps1"
...
function Register-KeePassVault {
    ...
    if ($ExistingVault) {
        ...
        # Write-Host "Keepass $VaultName is NOT Registered... Registering" -ForegroundColor Yellow
...
function Reset-IntuneEnrollment {
    ...
    # $clientId = "af8d1850-0bc5-4bc8-896d-bfc51b52f2ec" # Replace with your app registration client ID
    # $clientsecret = "'YOUR CLIENT SECRET'" # Replace with your app registration client ID
    # $tenantId = "f8e714f5-f15a-435d-b51e-9c93d637a9c4" # Replace with your Entra tenant ID
    ...
    # $graphAppCred = New-Object -TypeName System.Management.Automation.PSCredential -ArgumentList $clientId, $secureClientSecret
    # Connect-MgGraph -ClientId $clientId -TenantId $tenantId -Credential $graphAppCred
    ...
    #         Write-EnhancedLog -Message "Connected to Intune tenant $TenantId using app-based authentication (Azure AD authentication not supported)"
    #     } else {
    #         $graph = Connect-MSGraph -ea Stop
    #         Write-EnhancedLog -Message "Connected to Intune tenant $($graph.TenantId)"
    #     }
    # }
    ...
    # Write-EnhancedLog -Message "Checking actual Intune connection status" -ForegroundColor Cyan
    ...
    # Write-EnhancedLog -Message "Resetting Hybrid AzureAD connection" -ForegroundColor Cyan
    ...
    # Write-EnhancedLog -Message "Waiting" -ForegroundColor Cyan
    Write-EnhancedLog -Message "Waiting" -Level "INFO" -ForegroundColor ([ConsoleColor]::Cyan)

    Start-Sleep 10

    # Write-EnhancedLog -Message "Removing $computerName records from Intune" -ForegroundColor Cyan
    ...
    if (Get-Command Get-ADComputer -ErrorAction SilentlyContinue) {
        ...
        # Write-EnhancedLog -Message "AD module is missing, unable to obtain computer GUID"
    ...
    if ($IntuneObj) {
        $IntuneObj | Where-Object { $_ } | ForEach-Object {
            # Write-EnhancedLog -Message "Removing $($_.DeviceName) ($($_.id)) from Intune" -ForegroundColor Cyan
    ...
    # check certificates
    $i = 30
    # Write-EnhancedLog -Message "Waiting for Intune certificate creation"  -ForegroundColor Cyan
    ...
    if ($i -eq 0) {
        # Write-EnhancedLog -Message "Intune certificate (issuer: Microsoft Intune MDM Device CA) isn't created (yet?)"
        ...
        # Write-EnhancedLog -Message "DONE :)" -ForegroundColor Green
