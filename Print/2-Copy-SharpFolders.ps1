...
$mostRecentSharpFolders = Get-MostRecentSharpFolder
$mostRecentSharpFolders
# Now, $mostRecentSharpFolders contains the full paths of the most recent Sharp folders
...
<#
.SYNOPSIS
    Copies all folders and their descendant files from a specified source path to a target path, focusing on folders that contain 'Sharp' in their name.

.DESCRIPTION
    This function searches for folders matching the 'Sharp' pattern at the specified source path (including subdirectories), then copies these folders and all of their contents (files and subfolders) to the designated target path. It's designed to help streamline the process of relocating specific project folders to a new location.

.PARAMETER SourcePath
    The path where the function will search for 'Sharp' folders to copy. It includes all subdirectories in the search.

.PARAMETER TargetPath
    The base target path where the 'Sharp' folders and their contents will be copied. The function maintains the original folder structure within this target path.

.EXAMPLE
    Copy-SharpFolders -SourcePath "C:\Users\aollivierre\AppData\Local\Intune-Win32-Deployer" -TargetPath "C:\Code\CB\AppGallery\Intune-Win32-Deployer\Sharp"

    This example demonstrates how to use the Copy-SharpFolders function to copy all 'Sharp' folders from the specified source path to the specified target path, maintaining their original folder structure.
#>

function Copy-MostRecentSharpFoldersToTarget {
    Param (
        [string]$TargetPath = "C:\Code\CB\AppGallery\Intune-Win32-Deployer\Sharp"
    )

    $mostRecentSharpFolders = Get-MostRecentSharpFolder

    foreach ($folderFullPath in $mostRecentSharpFolders) {
...
Copy-MostRecentSharpFoldersToTarget