...
# Change Log by AOllivierre
# Replaced PS modules scope from current user to all users to avoid issues with PS modules saving in a OneDrive location/Documents folder
# Added the current script location to allow the script to run from VS Code and find the path for the run as admin script
#############################################################################################################
...
try{
    ...
    if(($install_choco -eq "y") -or ($install_winget -eq "y")){
        ...
        . "$d\source\ressources\prerequirements_AsAdmin.ps1" -choco $(if($install_choco -eq "y"){$true}else{$false}) -winget $(if($install_winget -eq "y"){$true}else{$false})
    }
}catch{$_}
