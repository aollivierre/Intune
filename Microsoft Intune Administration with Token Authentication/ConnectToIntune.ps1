...
function Connect-GraphWithToken {
    ...
    try {
        ...
        if ($context) {
            Write-Host "[SUCCESS] Successfully connected to Microsoft Graph PowerShell!" -ForegroundColor Green
            ...
            try {
                ...
                if ($device) {
                    Write-Host "[SUCCESS] Successfully verified access to Intune managed devices!" -ForegroundColor Green
                    return $true
                }
            }
            catch {
                Write-Host "[ERROR] Could not access Intune devices with Microsoft Graph cmdlets: $($_.Exception.Message)" -ForegroundColor Yellow
                return $false
            }
        }
        else {
            Write-Host "[ERROR] Could not establish Microsoft Graph PowerShell session" -ForegroundColor Red
            return $false
        }
    }
    catch {
        Write-Host "[ERROR] Error connecting to Microsoft Graph PowerShell: $($_.Exception.Message)" -ForegroundColor Red
