# Parameters
param(
    [string]$SignInLogsSource = "CSV", # Default to CSV. Other option could be "API"
    [string]$SignInLogsCsvFilePath = "C:\Users\aollivierre\Downloads\Bellwoods_InteractiveSignIns_2024-01-28_2024-02-27-v2.csv"
    # [string]$SignInLogsSource = "CSV"
)

# Your script logic follows here
Write-Host "SignInLogsSource: $SignInLogsSource"
Write-Host "SignInLogsCsvFilePath: $SignInLogsCsvFilePath"
...
Write-EnhancedLog -Message "Connected to Microsoft Graph." -Level "INFO" -ForegroundColor ([ConsoleColor]::Green)

# Assuming $SignInLogsSource and $SignInLogsCsvFilePath are defined earlier in the script...

function Get-IntuneDevicesWithPrimaryUserAndSignInLogs {
  $devices = Get-MgDeviceManagementManagedDevice -All
  Write-EnhancedLog -Message "Retrieved $($devices.Count) Intune-managed devices." -Level "INFO" -ForegroundColor ([ConsoleColor]::Green)

  $signInLogs = @()
  if ($SignInLogsSource -eq "CSV") {
      Write-EnhancedLog -Message "Loading sign-in logs from CSV file: $SignInLogsCsvFilePath" -Level "INFO" -ForegroundColor ([ConsoleColor]::Green)
      $signInLogs = Import-Csv -Path $SignInLogsCsvFilePath
  } else {
      Write-EnhancedLog -Message "Fetching all user sign-in logs from API..." -Level "INFO" -ForegroundColor ([ConsoleColor]::Green)
      $signInLogs = Get-MgAuditLogSignIn
  }

  $logCount = $signInLogs.Count
  Write-EnhancedLog -Message "Processed $logCount sign-in logs." -Level "INFO" -ForegroundColor ([ConsoleColor]::Green)

  $deviceUserSignInsList = New-Object System.Collections.Generic.List[Object]

  foreach ($device in $devices) {
      $primaryUser = Get-MgDeviceManagementManagedDevicePrimaryUser -DeviceId $device.Id
      $primaryUserId = $primaryUser.UserId

      if ($primaryUserId) {
          $userDetails = Get-MgUser -UserId $primaryUserId
          $primaryUserDisplayName = $userDetails.DisplayName

          # Find the latest sign-in log for this user
          $latestLog = $signInLogs | Where-Object { $_.'User ID' -eq $primaryUserId } | Sort-Object { [DateTime]::Parse($_.'Date (UTC)') } -Descending | Select-Object -First 1
          ...
          $deviceDetails = [PSCustomObject]@{
              ...
              LastSignInUserId       = $latestLog.'User ID'
              LastSignInTime         = $latestLog.'Date (UTC)'
          }
      } else {
          $deviceDetails = [PSCustomObject]@{
              DeviceId               = $device.Id
      ...
      $deviceUserSignInsList.Add($deviceDetails)
  }

  Write-EnhancedLog -Message "Processed $($deviceUserSignInsList.Count) devices for primary user sign-in details." -Level "INFO" -ForegroundColor ([ConsoleColor]::Green)
  return $deviceUserSignInsList
}
...
Write-EnhancedLog -Message "Function execution completed, displaying results." -Level "INFO" -ForegroundColor ([ConsoleColor]::Green)
