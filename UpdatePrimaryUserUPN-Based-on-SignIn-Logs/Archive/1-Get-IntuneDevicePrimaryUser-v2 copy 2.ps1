...
function Get-IntuneDevicesWithPrimaryUserAndSignInLogs {
    ...
    Write-EnhancedLog -Message "Retrieved $deviceCount Intune-managed devices." -Level "INFO" -ForegroundColor ([ConsoleColor]::Green)

    $allSignInLogs = Get-MgAuditLogSignIn
    $logCount = $allSignInLogs.Count
    Write-EnhancedLog -Message "Fetched $logCount user sign-in logs." -Level "INFO" -ForegroundColor ([ConsoleColor]::Green)

    # Using a generic list for improved performance
    $deviceUserSignIns = New-Object System.Collections.Generic.List[Object]

    foreach ($device in $devices) {
        $deviceDetails = New-Object PSObject -Property @{
            DeviceId = $device.Id
        ...
        if ($primaryUser -and $primaryUser.Count -gt 0) {
            ...
            if (-not [string]::IsNullOrWhiteSpace($primaryUserId)) {
                ...
                # Filter for the latest sign-in log
                $filteredLogs = $allSignInLogs | Where-Object { $_.UserId -eq $primaryUserId } | Sort-Object CreatedDateTime -Descending
                if ($filteredLogs -and $filteredLogs.Count -gt 0) {
                    $latestLog = $filteredLogs[0]
                    $deviceDetails.LastSignInUserId = $latestLog.UserId
                    $deviceDetails.LastSignInTime = $latestLog.CreatedDateTime
                }
            } else {
                Write-EnhancedLog -Message "No primary user ID found for device $($device.DeviceName)." -Level "WARNING" -ForegroundColor ([ConsoleColor]::Yellow)
            }
        }

        $deviceUserSignIns.Add($deviceDetails)
    }

    $userCount = $deviceUserSignIns.Count
    Write-EnhancedLog -Message "Processed $userCount devices for primary user sign-in details." -Level "INFO" -ForegroundColor ([ConsoleColor]::Green)
