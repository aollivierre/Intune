...
function Get-IntuneDevicesWithPrimaryUserAndSignInLogs {
  Write-EnhancedLog -Message "Retrieving all Intune-managed devices..." -Level "INFO" -ForegroundColor ([ConsoleColor]::Green)
  $devices = Get-MgDeviceManagementManagedDevice -All

  # Fetch all user sign-in logs
  Write-EnhancedLog -Message "Fetching all user sign-in logs..." -Level "INFO" -ForegroundColor ([ConsoleColor]::Green)
  ...
  foreach ($device in $devices) {
    ...
    if ($primaryUser -and $primaryUser.Count -gt 0) {
      ...
      if (-not [string]::IsNullOrWhiteSpace($primaryUserId)) {
        ...
        # Filter for the latest sign-in log for the primary user from the pre-fetched logs
        ...
        if ($filteredLogs -and $filteredLogs.Count -gt 0) {
          $latestLog = $filteredLogs | Select-Object -First 1
  ...
  Write-EnhancedLog -Message "Retrieved devices with user sign-ins." -Level "INFO" -ForegroundColor ([ConsoleColor]::Green)
...
Write-EnhancedLog -Message "Function execution completed." -Level "INFO" -ForegroundColor ([ConsoleColor]::Green)
