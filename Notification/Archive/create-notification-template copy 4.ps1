...
function Send-NotificationMessages {
    ...
    $displayNames = @("Immediately Alert 1", "7 Days Alert 2", "14 Days Alert 3", "21 Days Alert 4", "28 Days Alert 5")
     ...
     # Define the schedules and corresponding days for easier mapping
     $schedules = @{
        "Immediately" = 0
        "7 days" = 7
        "14 days" = 14
    ...
    foreach ($schedule in $schedules.GetEnumerator()) {
        $displayIndex = "{0:D3}" -f ($schedule.Value + 1) # Formatting index to 3 digits based on day value for uniqueness
        $displayName = "$DisplayNameBase $displayIndex - " + $schedule.Key

        # Check if display name exceeds 50 characters
        ...
        # Dynamically update the message with the number of days remaining
        $daysRemaining = $schedule.Value
        $dynamicMessagePart = if ($daysRemaining -eq 0) { "immediately" } else { "in $daysRemaining days" }
        $message = "$MessageBase Your access will be blocked $dynamicMessagePart."

        $subject = "$SubjectBase"

        # Your existing logic for creating the notification and notification message...
        # Ensure to use $message where you construct your JSON payloads

        # Write-Host "Notification Message: $message" # For debugging purposes
    }
